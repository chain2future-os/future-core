...
#include <futureio/sync_net_api_plugin/sync_net_api_plugin.hpp>
...
int main( int argc, char** argv ) {
   try {
      app().set_version(futureio::wss::config::version);
      appbase::app().register_plugin<sync_net_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "futureio/wssfuture/data" );
      app().set_default_config_dir(root / "futureio/wssfuture/config" );
      if( !appbase::app().initialize<http_plugin, sync_net_plugin,sync_net_api_plugin>( argc, argv ) )
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("wssfuture root is ${root}", ("root", root.string()));
      ilog("wssfuture version ${ver}", ("ver", futureio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      appbase::app().startup();
      appbase::app().exec();
   } catch ( const boost::exception& e ) {
