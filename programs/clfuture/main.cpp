 ...
 *  @defgroup futureclienttool FUTUREIO Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref nodfuture
 *  @ingroup futureclienttool
 */

/**
  @defgroup futureclienttool

  @section intro Introduction to clfuture

  `clfuture` is a command line tool that interfaces with the REST api exposed by @ref nodfuture. In order to use `clfuture` you will need to
  have a local copy of `nodfuture` running and configured to load the 'futureio::chain_api_plugin'.

   clfuture contains documentation for all of its commands. For a list of all commands known to clfuture, simply run it with no arguments:
```
$ ./clfuture
Command Line Interface to FUTUREIO Client
Usage: programs/clfuture/clfuture [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodfuture is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kfutured is running
...
Subcommands:
  ...
  transfer                    Transfer FGAS from account to account
...
Usage: ./clfuture create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./clfuture create account
Create a new account on the blockchain
Usage: ./clfuture create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <futureio/utilities/key_conversion.hpp>

#include <futureio/chain/name.hpp>
#include <futureio/chain/config.hpp>
#include <futureio/chain/wast_to_wasm.hpp>
...
using namespace futureio::chain;
using namespace futureio::utilities;
using namespace futureio::client::help;
using namespace futureio::client::http;
using namespace futureio::client::localize;
using namespace futureio::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define FUTUREC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
futureio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<futureio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return futureio::client::http::do_http_call( *sp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to nodfuture at ${u}; is nodfuture running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to kfutured at ${u}; is kfutured running?", ("u", url)) << std::endl;
...
futureio::chain_apis::read_only::get_info_results get_info() {
   return call(::url, get_info_func).as<futureio::chain_apis::read_only::get_info_results>();
}
...
void sign_transaction(signed_transaction& trx, fc::variant& required_keys, const chain_id_type& chain_id) {
   ...
   if (tx_multi_urls && addl_wallet_urls.size() > 0) {
      ...
      if (trx.signatures.size() != required_keys.as<flat_set<public_key_type>>().size()) {
         FUTURE_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets");
      }
   } else {
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   } FUTURE_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
fc::variant bin_to_json( const account_name& account, const action_name& action, const vector<char>& binargs ) {
   ...
   if( abi_serializer::to_abi(abi_v, abi) ) {
      ...
      FUTURE_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", account));
   }
   return result;
}
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active, bool updateable = false) {
   return action {
      ...
      futureio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = futureio::chain::authority{1, {{owner, 1}}, {}},
         .active       = futureio::chain::authority{1, {{active, 1}}, {}},
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } FUTURE_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "FUT") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } FUTURE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      FUTURE_ASSERT( futureio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<futureio::chain::symbol_code, futureio::chain::symbol> cache;
   auto a = asset::from_string( s );
   futureio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<futureio::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.insert(make_pair( sym, result.max_supply.get_symbol() ));
         it = p.first;
      } else {
         FUTURE_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      FUTURE_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( "futio.token", s );
}
...
void try_local_port( const string& lo_address, uint16_t port, uint32_t duration ) {
   ...
   while ( !local_port_used(lo_address, port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to kfutured, if kfutured is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kfutured")});
      }
   }
}

void make_kfutured_running(CLI::App* app, const string& url) {
    // get, version, net do not require kfutured
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by kfutured
          return;

    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running clfuture like this: ./clfuture ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("kfutured"); // if clfuture and kfutured are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kfutured").append("kfutured");
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kfuture(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kfuture.running()) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":" << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start kfutured because kfutured was not found." << std::endl;
    }
}

void ensure_kfutured_running(CLI::App* app) {
    make_kfutured_running(app, wallet_url);
    if (tx_multi_urls && addl_wallet_urls.size() > 0) {
        for (const auto& addl_url : addl_wallet_urls) {
            make_kfutured_running(app, addl_url);
        }
    }
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         if(superprivflg)
            permiss_info.push_back(permission_level{N(futureio),config::active_name});
...
struct create_account_subcommand {
   ...
   create_account_subcommand( CLI::App* actionRoot ) {
      ...
      createAccount->set_callback([this] {
            ...
            } FUTURE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } FUTURE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
   ...
   list_chain_types_subcommand(CLI::App* actionRoot) {
       ...
       list_chain_types->set_callback([this] {
          auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               ("table", "chaintypes") );
          auto chain_types = result.as<futureio::chain_apis::read_only::get_table_records_result>();
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<futureio::chain_apis::read_only::get_producers_result>();
         ...
         for ( auto& row : result.rows ){
            printf("%-13.13s %-54.54s  %-16ld  %-9s  %-.4f FGAS       %-12lu\n",
         ...
         printf("total_unpaid_balance: %-.4f FGAS \n",(double)total_unpaid_balance/10000);
...
struct delegate_cons_subcommand {
   ...
   delegate_cons_subcommand(CLI::App* actionRoot) {
      ...
      delegate_cons->add_option("stake_cons_quantity", stake_cons_amount, localized("The amount of FUT to stake for consensus weight"))->required();
...
struct list_delcons_subcommand {
   ...
   list_delcons_subcommand(CLI::App* actionRoot) {
      ...
      list_bw->set_callback([this] {
            ...
            if (!print_json) {
               auto res = result.as<futureio::chain_apis::read_only::get_table_records_result>();
               if ( !res.rows.empty() ) {
...
struct put_order_subcommand {
   ...
   put_order_subcommand(CLI::App* actionRoot) {
      ...
      put_order->set_callback([this] {
         ...
         send_actions({create_action(permiss_info, N(futio.res), NEX(putorder), act_payload)});
      });
   }
};
...
struct list_orders_subcommand {
   ...
   list_orders_subcommand(CLI::App* actionRoot) {
      ...
      list_orders->set_callback([this] {
         ...
         auto result = rawResult.as<vector<futureio::chain_apis::read_only::get_resource_orders_result>>();
         if(result.empty()) {
         ...
         for ( const auto& row : result ) {
            printf("%-13.13s %-8lu    %-10d  %-9.4f FGAS    %-15.5s     %-9.4f FGAS\n",
...
struct update_order_subcommand {
   ...
   update_order_subcommand(CLI::App* actionRoot) {
      ...
      update_order->set_callback([this] {
         ...
         vector<permission_level> permiss_info;
         permiss_info.push_back(permission_level{owner_account,config::active_name});
         send_actions({create_action(permiss_info, N(futio.res), NEX(updateorder), act_payload)});
      });
   }
};
...
struct cancel_order_subcommand {
   ...
   cancel_order_subcommand(CLI::App* actionRoot) {
      ...
      cancel_order->set_callback([this] {
         ...
         vector<permission_level> permiss_info;
         permiss_info.push_back(permission_level{owner_account,config::active_name});
         send_actions({create_action(permiss_info, N(futio.res), NEX(cancelorder), act_payload)});
      });
   }
};
...
struct buy_in_subcommand {
   ...
   buy_in_subcommand(CLI::App* actionRoot) {
      ...
      buy_in->set_callback([this] {
         ...
         send_actions({create_action(permiss_info, N(futio.res), NEX(buyin), act_payload)});
      });
   }
};
   ...
   buy_resource_package_subcommand(CLI::App* actionRoot) {
      ...
      buy_respackage->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("period", period_id)
                  ("location", location);
         std::vector<chain::action> acts{create_action({permission_level{from_str,config::active_name}}, N(futio.res), NEX(resourcelease), act_payload)};
         send_actions(std::move(acts));
      });
   }
};
   ...
   transfer_resource_package_subcommand(CLI::App* actionRoot) {
      ...
      transfer_respackage->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("combosize", combosize)
                  ("period", period_id);
         std::vector<chain::action> acts{create_action({permission_level{from_str,config::active_name}}, N(futio.res), NEX(transresource), act_payload)};
         send_actions(std::move(acts));
      });
   }
};
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<futureio::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, futureio::chain_apis::permission> cache;
      ...
      if ( res.refund_cons.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'futureio::refundcons' action)\n";
            } else {
...
int main( int argc, char** argv ) {
   ...
   context = futureio::client::http::create_http_context();

   fc::microseconds abi_serializer_max_time = fc::seconds(1); // No risk to client side serialization taking a long time

   CLI::App app{"Command Line Interface to FUTIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where nodfuture is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where nodfuture is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kfutured is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kfutured is running") )->group("hidden");

   app.add_option( "-u,--url", ::url, localized("the http/https URL where nodfuture is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where kfutured is running"), true );
   ...
   app.set_callback([&app]{ ensure_kfutured_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", futureio::client::config::version_str)) << std::endl;
   });
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         //see if this is an old nodfuture that doesn't support get_raw_code_and_abi
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older nodfuture which returns malformed binary wasm") << std::endl;
         }
         else
   ...
   getAccounts->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } FUTURE_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } FUTURE_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      FUTURE_ASSERT( !wast.empty(), wast_file_not_found, "no wast file found ${f}", ("f", wastPath) );
      vector<uint8_t> wasm;
   ...
   auto set_abi_callback = [&]() {
      ...
      FUTURE_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );

      try {
         actions.emplace_back( create_setabi(account, fc::json::from_file(abiPath).as<abi_def>()) );
      } FUTURE_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   // Transfer subcommand
   string con = "futio.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer FGAS from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending FGAS"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving FGAS"))->required();
   transfer->add_option("amount", amount, localized("The amount of FGAS to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         FUTURE_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         FUTURE_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKfutured = wallet->add_subcommand("stop", localized("Stop kfutured (doesn't work with nodfuture)."), false);
   stopKfutured->set_callback([] {
      const auto& v = call(wallet_url, kfutured_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kfutured responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from nodfuture" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } FUTURE_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_data))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_data_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            FUTURE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "futio.msig", "propose", variant_to_bin( N(futio.msig), NEX(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", futureio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto args = fc::mutable_variant_object()
         ("proposer", proposer)
         ("proposal_name", proposal_name)
         ("level", perm_var);
      ...
      send_actions({chain::action{accountPermissions, "futio.msig", action, variant_to_bin( N(futio.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            FUTURE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "futio.msig", "cancel", variant_to_bin( N(futio.msig), NEX(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            FUTURE_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "futio.msig", "exec", variant_to_bin( N(futio.msig), NEX(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))

      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"futio.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "futio.sudo", "exec", variant_to_bin( N(futio.sudo), NEX(exec), args ) }});
   });

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send futureio.system contract action to the blockchain."), false);
