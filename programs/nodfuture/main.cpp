...
{
   try {
      app().set_version(futureio::nodfuture::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "futureio/nodfuture/data" );
      app().set_default_config_dir(root / "futureio/nodfuture/config" );
      ...
      initialize_logging();
      ilog("nodfuture version ${ver}", ("ver", futureio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("futureio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
