...
namespace futureio {
    ...
    void NodeInfo::setCommitteeInfo(const std::vector<std::string>& accountV, const std::vector<std::string>& skV,
        ...
        FUTURE_ASSERT(accountV.size() != 0, chain::chain_exception, "account is empty");
        FUTURE_ASSERT(accountV.size() == skV.size() && skV.size() == blsSkV.size() && blsSkV.size() == accTrxPriKeyV.size(),
        ...
        for (size_t i = 0; i < accountV.size(); i++) {
            ...
            FUTURE_ASSERT(blsSkV[i].length() == Bls::BLS_PRI_KEY_LENGTH * 2, chain::chain_exception, "bls private key error");
        ...
        if (m_accountV.size() == 1 && Genesis::kGenesisAccount == m_accountV[0]) {
            FUTURE_ASSERT(m_privateKeyV[0].getPublicKey() == consensus::PublicKeyType(Genesis::s_genesisPk),
                           chain::chain_exception,
