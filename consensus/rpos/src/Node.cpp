...
namespace futureio {
    ...
    Node::Node(boost::asio::io_service& ioservice) : m_ready(false), m_connected(false), m_syncing(false),
        ...
        std::string ver =
            futureio::utilities::common::itoh(static_cast<uint32_t>(app().version())) + "+" + chain::get_version_str();
    ...
    void Node::vote(uint32_t blockNum, ConsensusPhase phase, uint32_t baxCount) {
        ...
        if (kPhaseBA0 == phase) {
            ...
            if (MsgMgr::getInstance()->hasProposer(blockNum, index)) {
                ...
                FUTURE_ASSERT(ret, chain::chain_exception, "Init propose msg failed");
                ...
                if (MsgMgr::getInstance()->hasVoter(getBlockNum(), kPhaseBA0, 0, v)) {
                    for (auto index : v) {
                        ...
                        FUTURE_ASSERT(m_schedulerPtr->verifyMyBlsSignature(echo, index), chain::chain_exception, "bls signature error, check bls private key pls");
    ...
    void Node::ba1Process() {
        ...
        FUTURE_ASSERT(blockPtr->id() == m_schedulerPtr->getPreviousBlockhash(), chain::chain_exception,
    ...
    void Node::baxProcess() {
        ...
        if (!isBlank(baxBlock.id())) {
            ...
            *blockPtr = baxBlock;
            ...
            FUTURE_ASSERT(blockPtr->id() == m_schedulerPtr->getPreviousBlockhash(),
    ...
    bool Node::syncFail(const futureio::ReqSyncMsg& sync_msg) {
        m_syncFailed = true;
        m_ready = true;
        m_syncing = false;
    ...
    void Node::sendEchoForEmptyBlock(const std::vector<size_t>& v) {
        ...
        for (auto index : v) {
            ...
            FUTURE_ASSERT(m_schedulerPtr->verifyMyBlsSignature(echoMsg, index), chain::chain_exception, "bls signature error, check bls private key pls");
