...
#define THRESHOLD_STAKE 6000
    ...
    void StakeVoteVrf::init() {
        ...
        FUTURE_ASSERT(totalStake != 0, chain::chain_exception, "totalStake is 0");
    ...
    void StakeVoteVrf::moveToNewStep(uint32_t blockNum, ConsensusPhase phase, int baxCount) {
        FUTURE_ASSERT(blockNum == m_blockNum, chain::chain_exception, "blockNum is not equal");
    ...
    Proof StakeVoteVrf::getVoterProof(uint32_t blockNum, ConsensusPhase phase, int baxCount) {
        FUTURE_ASSERT(blockNum == m_blockNum, chain::chain_exception, "blockNum is not equal");
        FUTURE_ASSERT(m_phaseProofMap.find(phase + baxCount) != m_phaseProofMap.end(), chain::chain_exception, "should be implemented by subclass");
        return m_phaseProofMap[phase + baxCount];
    }

    Proof StakeVoteVrf::getProposerProof(uint32_t blockNum) {
        FUTURE_ASSERT(blockNum == m_blockNum, chain::chain_exception, "blockNum is not equal");
        FUTURE_ASSERT(m_proposerProof.isValid(), chain::chain_exception, "should be implemented by subclass");
