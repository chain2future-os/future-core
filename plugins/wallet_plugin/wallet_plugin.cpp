 ...
 */
#include <futureio/wallet_plugin/wallet_plugin.hpp>
#include <futureio/wallet_plugin/wallet_manager.hpp>
#include <futureio/wallet_plugin/yubihsm_wallet.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("futureio-key", bpo::value<std::string>(),
          "futureio key that will be imported automatically when a wallet is created.")
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      ...
      if (options.count("unlock-timeout")) {
         ...
         FUTURE_ASSERT(timeout > 0, chain::invalid_lock_timeout_exception, "Please specify a positive timeout ${t}", ("t", timeout));
	  ...
	  if (options.count("futureio-key")) {
        std::string futureio_wif_key = options.at("futureio-key").as<std::string>();
        wallet_manager_ptr->set_futureio_key(futureio_wif_key);
       }
   } FC_LOG_AND_RETHROW()
}
