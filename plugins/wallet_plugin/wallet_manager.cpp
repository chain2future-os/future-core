 ...
 */
#include <futureio/wallet_plugin/wallet_manager.hpp>
#include <futureio/wallet_plugin/wallet.hpp>
#include <futureio/wallet_plugin/se_wallet.hpp>
...
void wallet_manager::set_timeout(const std::chrono::seconds& t) {
   ...
   FUTURE_ASSERT(timeout_time >= now, invalid_lock_timeout_exception, "Overflow on timeout_time, specified ${t}, now ${now}, timeout_time ${timeout_time}",
...
std::string wallet_manager::create(const std::string& name) {
   ...
   FUTURE_ASSERT(valid_filename(name), wallet_exception, "Invalid filename, path not allowed in wallet name ${n}", ("n", name));

   auto wallet_filename = dir / (name + file_ext);

   if (fc::exists(wallet_filename)) {
      FUTURE_THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path",fc::path(wallet_filename)));
   }
   ...
   if(futureio_key.size())
      wallet->import_key(futureio_key);
   wallet->lock();
   wallet->unlock(password);
   ...
   // This can happen if the wallet file is removed while future-walletd is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   if (!wallet->load_wallet_file()) {
      FUTURE_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
   }

   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while future-walletd is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
...
map<public_key_type,private_key_type> wallet_manager::list_keys(const string& name, const string& pw) {
   ...
   if (wallets.count(name) == 0)
      FUTURE_THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
   auto& w = wallets.at(name);
   if (w->is_locked())
      FUTURE_THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
...
flat_set<public_key_type> wallet_manager::get_public_keys() {
   check_timeout();
   FUTURE_ASSERT(!wallets.empty(), wallet_not_available_exception, "You don't have any wallet!");
   ...
   FUTURE_ASSERT(!is_all_wallet_locked, wallet_locked_exception, "You don't have any unlocked wallet!");
   return result;
}
...
void wallet_manager::unlock(const std::string& name, const std::string& password) {
   ...
   auto& w = wallets.at(name);
   if (!w->is_locked()) {
      FUTURE_THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
      return;
   }
   w->unlock(password);
}
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
   ...
   for (const auto& pk : keys) {
      ...
      if (!found) {
         FUTURE_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
      }
   }
   ...
   if (interactive_mode == true) {
      ...
      while(true) {
         ...
         if ( boost::iequals(choice, "n") || boost::iequals(choice, "no")) {
            FUTURE_THROW(chain::wallet_exception, "Transaction sign rejected!");
            break;
...
wallet_manager::sign_digest(const chain::digest_type& digest, const public_key_type& key) {
   ...
   FUTURE_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", key));
}
