...
#include <futureio/net_plugin/net_plugin.hpp>
...
namespace futureio { namespace net_plugin_n {
    ...
    public:
        ...
        void handle_message( connection_ptr c, const futureio::EchoMsg &msg);
        void handle_message( connection_ptr c, const futureio::ProposeMsg& msg);
        void handle_message( connection_ptr c, const futureio::ReqBlockNumRangeMsg& msg);
        void handle_message( connection_ptr c, const futureio::RspBlockNumRangeMsg& msg);
        void handle_message( connection_ptr c, const futureio::ReqSyncMsg& msg);
        void handle_message( connection_ptr c, const futureio::SyncBlockMsg& msg);
        void handle_message( connection_ptr c, const futureio::SyncStopMsg& msg);
        ...
        bool send_req_sync(const futureio::ReqSyncMsg& msg);
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
    ...
    public:
        ...
        futureio::ReqSyncMsg            sync_block_msg;
            ...
            for (auto& con : rsp_conns) {
                if (con->block_num_range.firstNum == 0 || con->block_num_range.firstNum > sync_block_msg.startBlockNum) { // can't provide all the blocks
                    ...
                    if (conns_without_block.size() == sync_src_count && seq_num >= 15) {
                        FUTURE_ASSERT(false, chain::unlinkable_block_exception, "No block in neighbors. Help!!!");
                    }
...
connection::connection(string endpoint, msg_priority pri, connection_direction dir)
    ...
    bool net_plugin_impl::send_req_sync(const futureio::ReqSyncMsg& msg) {
            ...
            [this,weak_conn]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     FUTURE_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
...
//            ("phase", (uint32_t)msg.phase)("baxcount",msg.baxCount)("account", std::string(msg.account))("sig", short_sig(msg.signature)));
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const ProposeMsg& msg) {
    ...
    void net_plugin_impl::handle_message( connection_ptr c, const futureio::ReqBlockNumRangeMsg& msg) {
    ...
    void net_plugin_impl::handle_message( connection_ptr c, const futureio::RspBlockNumRangeMsg& msg) {
    ...
    void net_plugin_impl::handle_message( connection_ptr c, const futureio::ReqSyncMsg& msg) {
    ...
    void net_plugin_impl::handle_message( connection_ptr c, const futureio::SyncBlockMsg& msg) {
    ...
    void net_plugin_impl::handle_message( connection_ptr c, const futureio::SyncStopMsg &msg) {
   ...
   void net_plugin_impl::connection_monitor( ) {
      ...
      while(it != connections.end()) {
         if (!(*it)->socket->is_open() && !(*it)->connecting) {
	       ...
	       if((*it)->no_retry == go_away_reason::duplicate){
		    it = connections.erase(it);
		    continue;
	       }else{
		    connect(*it);
...
bool net_plugin_impl::authenticate_peer(const handshake_message& msg) {
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"FUTURE Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         FUTURE_ASSERT( my->max_static_clients + my->max_dynamic_clients > my->min_connections, plugin_config_exception, "max_client_count must be > min_connections");
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            FUTURE_ASSERT( options.count( "peer-key" ),
   ...
   void net_plugin::send_block(const fc::sha256& node_id, const futureio::SyncBlockMsg& msg) {
   ...
   bool net_plugin::send_req_sync(const futureio::ReqSyncMsg& msg) {
       ilog("send req sync msg");
       return my->send_req_sync(msg);
   }

   void net_plugin::send_block_num_range(const fc::sha256& node_id, const futureio::RspBlockNumRangeMsg& msg) {
