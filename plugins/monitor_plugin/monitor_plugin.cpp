 ...
 */
#include <futureio/monitor_plugin/monitor_plugin.hpp>
...
fc::variant call( const std::string& url,
   ...
   try {
       ...
       auto sp = std::make_unique<futureio::client::http::connection_param>(context, parse_url(url) + path,  false, headers);
       return futureio::client::http::do_http_call( *sp, fc::variant(v), false, false );
   }
...
void monitor_plugin_impl::processReportTask() {
  ...
  } //don't allow exception be thrown out, to prevent Futureode from exiting.
}
...
monitor_apis::monitor_only  monitor_plugin::get_monitor_only_api()const {
   ...
   namespace monitor_apis {
     ...
     const std::shared_ptr<Node>  monitor_only::getNodePtr() const {
       ...
       FUTURE_ASSERT( nodePtr != nullptr, chain::node_not_found_exception, "Failed to get Node. Most possible reason: This Node is not a producer." );
       return nodePtr;
     }
