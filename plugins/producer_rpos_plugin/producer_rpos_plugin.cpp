 ...
 */
#include <futureio/producer_rpos_plugin/producer_rpos_plugin.hpp>
...
#include <futureio/chain/merkle.hpp>
...
namespace {
    bool failure_is_subjective(const fc::exception& e) {
        auto code = e.code();
        return ((code == futureio::chain::block_cpu_usage_exceeded::code_value) ||
                (code == futureio::chain::block_net_usage_exceeded::code_value));
    }
}
...
class producer_rpos_plugin_impl : public std::enable_shared_from_this<producer_rpos_plugin_impl> {
   public:
      ...
      bool     _startup_multithread_verify_sig     = false;
      ...
      fc::microseconds                                          _kfutured_provider_timeout_us;
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("startup-multithread-verify-sig", boost::program_options::bool_switch()->notifier([this](bool e){my->_startup_multithread_verify_sig = e;}), "startup multithread to verify signature for producer")
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild FUTUREIO public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KFUTURED\n\n"
          "   KEY:<data>      \tis a string form of a valid FUTUREIO private key which maps to the provided public key\n\n"
          "   KFUTURED:<data>    \tis the URL where kfutured is available and the approptiate wallet(s) are unlocked")
         ("kfutured-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowd for sending blocks to a kfutured provider for signing")
...
static producer_rpos_plugin_impl::signature_provider_type
make_kfutured_signature_provider(const std::shared_ptr<producer_rpos_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   auto kfutured_url = fc::url(url_str);
   std::weak_ptr<producer_rpos_plugin_impl> weak_impl = impl;

   return [weak_impl, kfutured_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kfutured_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kfutured_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().post_sync(kfutured_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            } else if (spec_type_str == "KFUTD") {
               my->_signature_providers[pubkey] = make_kfutured_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kfutured_provider_timeout_us = fc::milliseconds(options.at("kfutured-provider-timeout").as<int32_t>());
   ...
   futureio::chain::config::block_interval_ms = my->_max_round_seconds * 1000;
   futureio::chain::config::block_interval_us =  my->_max_round_seconds * 1000000;
...
bool producer_rpos_plugin::sync_fail(const futureio::ReqSyncMsg& sync_msg) {
...
{ try {
   ...
   app().get_plugin<chain_plugin>().chain().startup_sig_verify_thread(my->_is_non_producing_node, my->_startup_multithread_verify_sig);
