...
#include <futureio/chain/fork_database.hpp>
#include <futureio/chain/block_log.hpp>
...
#include <futureio/chain/generated_transaction_object.hpp>
...
std::ostream& operator<<(std::ostream& osm, futureio::chain::db_read_mode m) {
   if ( m == futureio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == futureio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == futureio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              futureio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(futureio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(futureio::chain::db_read_mode::HEAD);
  } else if ( s == "irreversible" ) {
     v = boost::any(futureio::chain::db_read_mode::IRREVERSIBLE);
  } else {
...
using namespace futureio::chain::config;
using namespace futureio::chain::plugin_interface;
...
using futureio::LightClient;
using futureio::CommitteeSet;
using futureio::LightClientMgr;
using futureio::Evidence;
using futureio::EvidenceFactory;
using futureio::evildoer;

class light_client_callback : public futureio::chain::callback {
public:
    ...
    bool getUnconfirmedHeaderFromDb(const chain::name &chainName, std::vector<signed_block_header> &unconfirmedBlockHeader, StartPoint& startPoint) {
        try {
            ...
            if (result.genesisPk.empty()) {
                result.genesisPk = std::string("FUT7fzCYqCvi5WiAq6sVWqhqwcJ9UXuefASqptqa76hLV4gfU7LdD"); // default
            }
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("read-mode", boost::program_options::value<futureio::chain::db_read_mode>()->default_value(futureio::chain::db_read_mode::SPECULATIVE),
...
#endif
         ("register-events", bpo::value<vector<string>>()->composing()->multitoken(), "register events as account + post url")
         ;
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   futureio::chain::callback_manager::get_self()->register_callback(std::make_shared<light_client_callback>());
   try {
      try {
         genesis_state gs; // Check if FUTUREIO_ROOT_KEY is bad
      } catch ( const fc::exception& ) {
         elog( "FUTUREIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::futureio_root_key));
         throw;
      }
     futureio::chain::config::default_max_block_cpu_usage = options.at("max_block_cpu_usage").as<uint32_t>();
     futureio::chain::config::default_max_transaction_cpu_usage = futureio::chain::config::default_max_block_cpu_usage / 2;
     futureio::chain::config::default_max_block_net_usage = options.at("max_block_net_usage").as<uint32_t>();
     futureio::chain::config::default_max_transaction_net_usage = futureio::chain::config::default_max_block_net_usage / 2;
     ilog("default ${default_max_block_cpu_usage}",("default_max_block_cpu_usage",futureio::chain::config::default_max_block_cpu_usage));
      my->chain_config = controller::config();
      if(options.count("genesis-time"))
      {
          my->_genesis_time = options.at( "genesis-time" ).as<string>();
      }
      FUTURE_ASSERT( !my->_genesis_time.empty(),
      ...
      FUTURE_ASSERT( !my->_chain_name.empty(),
              plugin_config_exception,
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               FUTURE_ASSERT( itr->second == item.second,
                           plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         FUTURE_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if (options.count( "worldstate" )) {
         ...
         FUTURE_ASSERT( fc::exists(*my->worldstate_path), plugin_config_exception,
         ...
         FUTURE_ASSERT( options.count( "genesis-json" ) == 0 &&  options.count( "genesis-timestamp" ) == 0,
                 plugin_config_exception,
         ...
         FUTURE_ASSERT( !fc::exists(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            FUTURE_ASSERT( log_genesis.compute_chain_id() == my->chain_config->genesis.compute_chain_id(),
                    plugin_config_exception,
         ...
         if( options.count( "genesis-json" )) {
            FUTURE_ASSERT( !fc::exists( my->blocks_dir / "blocks.log" ),
                        plugin_config_exception,
            ...
            FUTURE_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            wlog( "Starting up fresh blockchain with provided genesis state." );
             ...
             FUTURE_ASSERT( genesis_timestamp == my->chain_config->genesis.initial_timestamp,
                     plugin_config_exception,
      ...
      if ( options.count("read-mode") ) {
         ...
         FUTURE_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported." );
      }
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            FUTURE_ASSERT( itr->second == id, checkpoint_exception,
      ...
      if ( options.count("register-events") ) {
         std::vector<std::string> events = options["register-events"].as<std::vector<std::string>>();
         for (auto& e : events) {
            auto pos = e.find(',');
            if (pos == std::string::npos) {
                elog("register-events format error.");
                continue;
            }
            auto account = e.substr(0, pos);
            auto post_url = e.substr(pos + 1, e.size());
            ilog("register event: ${a}, ${p}", ("a", account)("p", post_url));
            my->chain->register_event(account, post_url);
         }
      }
...
chain::chain_id_type chain_plugin::get_chain_id()const {
   FUTURE_ASSERT( my->chain_id.valid(), chain_id_type_exception, "chain ID has not been initialized yet" );
   return *my->chain_id;
}
...
read_only::get_info_results read_only::get_chain_info(const read_only::get_chain_info_params&) const {
   ...
   return {
      futureio::utilities::common::itoh(static_cast<uint32_t>(app().version())) + "+" + futureio::chain::get_version_str(),
      db.get_chain_id(),
...
uint64_t read_only::get_table_index_name(const read_only::get_table_records_params& p, bool& primary) {
   ...
   FUTURE_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         FUTURE_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   try {
      ...
      try {
         ...
         try {
            auto symb = futureio::chain::symbol::from_string(str);
            value = symb.value();
         } catch( ... ) {
            try {
               value = ( futureio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
            } catch( ... ) {
               FUTURE_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
uint64_t convert_to_scope( const string& str, const string& type ) {
   ...
   if ( type == "symbol" || is_default_convert ) {
      try {
         ...
         try {
            return ( futureio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
         } catch( ... ) {}
      }
   }
   ...
   FUTURE_ASSERT( false, chain_type_exception, "convert_to_scope could not convert scope string '${str}' to any of the following: "
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   FUTURE_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   FUTURE_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_records_result read_only::get_table_records( const read_only::get_table_records_params& p )const {
   const abi_def abi = futureio::chain_apis::get_abi( db, p.code );

   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      FUTURE_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_records_ex<key_value_index>(p,abi);
      }
      FUTURE_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      FUTURE_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      FUTURE_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      FUTURE_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      FUTURE_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( futureio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, scope, N(stat), [&](const key_value_object& obj){
      FUTURE_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
vector<string> read_only::get_node_account( const get_node_account_params& ) const {
   return NodeInfo::getInstance()->getAccountList();
}
...
vector<read_only::get_subchain_committee_result> read_only::get_subchain_committee( const read_only::get_subchain_committee_params& p )const {
   const abi_def abi = futureio::chain_apis::get_abi( db, N(futureio) );
   FUTURE_ASSERT(p.chain_name != self_chain_name, chain::contract_table_query_exception, "Could not query committee list of master chain.");

   name table = N(producers);
   auto index_type = get_table_type( abi, table );

   vector<get_subchain_committee_result> result;
   walk_key_value_table(N(futureio), p.chain_name, table, [&](const key_value_object& obj){
//       FUTURE_ASSERT( obj.value.size() >= sizeof(futureio::chain::subchain), chain::asset_type_exception, "Invalid subchain data on table");

       futureio::chain::producer_info producer_data;
       fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
...
read_only::get_subchain_block_num_result read_only::get_subchain_block_num(const read_only::get_subchain_block_num_params& p) const {
   ...
   walk_key_value_table(N(futureio), N(futureio), table, [&](const key_value_object& obj){
   //    FUTURE_ASSERT( obj.value.size() >= sizeof(subchain), chain::asset_type_exception, "Invalid subchain data on table");
...
read_only::get_master_block_num_result read_only::get_master_block_num(const read_only::get_master_block_num_params& p) const {
   ...
   walk_key_value_table(N(futureio), N(futureio), table, [&](const key_value_object& obj){
   //    FUTURE_ASSERT( obj.value.size() >= sizeof(chain_info), chain::asset_type_exception, "Invalid master data on table");
...
static fc::variant get_global_row( const database& db, const abi_def& abi, const fc::microseconds& abi_serializer_max_time_ms ) {
   ...
   FUTURE_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));

   const auto* const table_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(futureio), N(futureio), N(global)));
   FUTURE_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   FUTURE_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   ...
   FUTURE_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   if(p.all_chain) {
       ...
       //add side chains
       walk_key_value_table(N(futureio), N(futureio), N(chains), [&](const key_value_object& obj){
           futureio::chain::chain_info subchain_data;
           fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
           fc::raw::unpack(ds, subchain_data);
   ...
   for(auto i = 0; i < scopes.size(); ++i) {
       if(scopes[i] == N(master))
           continue;
       const auto* const table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(futureio), scopes[i], N(producers)));
       const auto* const secondary_table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(futureio), scopes[i], N(producers) | secondary_index_num));
       if(table_id == nullptr || secondary_table_id == nullptr) {
           FUTURE_ASSERT(scopes[i] != self_chain_name, chain::contract_table_query_exception, "Missing master producers table");
           continue;
       }
...
static fc::variant get_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms, const name& key) {
    ...
    FUTURE_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table rand", ("type",table_type));

    const auto* const table_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(futio.rand), N(rand), N(rand)));
    if (table_id == nullptr) {
        return fc::variants();
    }
...
read_only::get_random_result read_only::get_random(const read_only::get_random_params& p) const {
    const abi_def abi = futureio::chain_apis::get_abi(db, N(futio.rand));
    const auto table_type = get_table_type(abi, N(rand));
    const abi_serializer serializer { abi, abi_serializer_max_time };
    FUTURE_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table rand", ("type", table_type));
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  FUTURE_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block_info(const read_only::get_block_info_params& params) const {
   ...
   FUTURE_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64, chain::block_id_type_exception, "Invalid Block number or ID, must be greater than 0 and less than 64 characters" );
   try {
   ...
   } FUTURE_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   FUTURE_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
std::vector<read_only::get_resource_orders_result> read_only::get_resource_orders(const read_only::get_resource_orders_params& params) {
   const abi_def abi = futureio::chain_apis::get_abi( db, N(futio.res) );
   ...
   walk_key_value_table(N(futio.res), scope_i, table, [&](const key_value_object& obj){
       futureio::chain::resource_sale resorder_data;
       fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   FUTURE_ASSERT( b, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_tx(const read_write::push_tx_params& params, next_function<read_write::push_tx_results> next) {
   ...
   try {
      ...
      } FUTURE_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_txs(const read_write::push_txs_params& params, next_function<read_write::push_txs_results> next) {
   try {
      FUTURE_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_account_results read_only::get_account_info( const get_account_info_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            FUTURE_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   const auto& code_account = db.db().get<account_object,by_name>( N(futureio) );
   const auto& res_account = db.db().get<account_object,by_name>( N(futio.res) );
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) && abi_serializer::to_abi(res_account.abi, res_abi)) {
      ...
      const auto token_code = N(futio.token);
...
read_only::trans_fee_result read_only::get_trans_fee( const get_trans_fee_params& params ) const {
    FUTURE_ASSERT( params.block_height != 0, transaction_exception, "get_trans_fee block_height must be greater than 0" );
...
read_only::abi_json2bin_result read_only::abi_json2bin( const read_only::abi_json2bin_params& params )const try {
   ...
   FUTURE_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      FUTURE_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary(action_type, params.args, abi_serializer_max_time);
      } FUTURE_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      FUTURE_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } FUTURE_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
#ifdef ENABLE_ZKP
read_only::get_anchors_result read_only::get_anchors( const get_anchors_params& p ) const {
   get_anchors_result result;

   const abi_def abi = futureio::chain_apis::get_abi( db, p.code );
   auto index_type = get_table_type( abi, N(anchors) );
   //ilog("code: ${c} scope: ${s} start: ${st} end: ${e}", ("c", p.code)("s", p.scope)("st", p.start)("e", p.end));
   walk_key_value_table(p.code, p.scope, N(anchors), [&](const key_value_object& obj){
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      anchors_data data;
      ...
      for (uint32_t i = data.first; i < data.anchors.size(); ) {
         // query pos range = [start, end)
         if (data.anchors[i].pos < p.start) {
         } else if (data.anchors[i].pos >= p.end) {
            break;
         } else {
            result.anchors.push_back(data.anchors[i]);
         }

         if ((i + 1)%data.anchors.size() == data.first) {
            break;
         } else if (i + 1 == data.anchors.size()) {
            i = 0;
         } else {
            i++;
         }
