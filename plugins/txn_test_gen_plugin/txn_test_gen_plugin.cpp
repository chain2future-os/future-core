 ...
 */
#include <futureio/txn_test_gen_plugin/txn_test_gen_plugin.hpp>
...
#include <futureio.token/futureio.token.wast.hpp>
#include <futureio.token/futureio.token.abi.hpp>
#include <hello/hello.wast.hpp>
#include <hello/hello.abi.hpp>

namespace futureio { namespace detail {
  struct txn_test_gen_empty {};
}}

FC_REFLECT(futureio::detail::txn_test_gen_empty, );
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     futureio::detail::txn_test_gen_empty result;
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [cb, body](const fc::exception_ptr& e) {\
         if (e) {\
            ...
            cb(http_response_code, fc::json::to_string(futureio::detail::txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(futureio_token_abi).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer futureio_token_serializer{fc::json::from_string(futureio_token_abi).as<abi_def>(), abi_serializer_max_time};
         ...
         //create some test accounts
         {
            ...
            //create "A" account
            {
            auto owner_auth   = futureio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            auto active_auth  = futureio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            ...
            //create "B" account
            {
            auto owner_auth   = futureio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            auto active_auth  = futureio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            ...
            {
            auto owner_auth   = futureio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
            auto active_auth  = futureio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
         ...
         //set txn.test.t contract to futureio.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = wast_to_wasm(std::string(futureio_token_wast));
...

            {
               setabi handler;
               handler.account = newaccountC;
               handler.abi = fc::raw::pack(json::from_string(futureio_token_abi).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountC,"active"}}, handler);
            }

            {
               ...
               act.data = futureio_token_serializer.variant_to_binary("create", fc::json::from_string("{\"issuer\":\"txn.test.t\",\"maximum_supply\":\"1000000000.0000 CUR\"}}"), abi_serializer_max_time);
            ...
            }
            {
               ...
               act.name = NEX(issue);
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = futureio_token_serializer.variant_to_binary("issue", fc::json::from_string("{\"to\":\"txn.test.t\",\"quantity\":\"600.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
            {
               action act;
               ...
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = futureio_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"txn.test.t\",\"to\":\"txn.test.a\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
            {
               action act;
               ...
               act.name = NEX(transfer);
               act.authorization = vector<permission_level>{{newaccountC,config::active_name}};
               act.data = futureio_token_serializer.variant_to_binary("transfer", fc::json::from_string("{\"from\":\"txn.test.t\",\"to\":\"txn.test.b\",\"quantity\":\"200.0000 CUR\",\"memo\":\"\"}"), abi_serializer_max_time);
               trx.actions.push_back(act);
            }
   ...
   void start_generation_transfer(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
     ...
     //    throw fc::exception(fc::invalid_operation_exception_code);
      ...
      act_a_to_b.account = N(futio.token);
      act_a_to_b.name = NEX(transfer);
      act_a_to_b.authorization = vector<permission_level>{{name("user.111"),config::active_name}};
      act_a_to_b.data = futureio_token_serializer.variant_to_binary("transfer",
                                                                  fc::json::from_string(fc::format_string("{\"from\":\"user.111\",\"to\":\"user.112\",\"quantity\":\"0.1234 FGAS\",\"memo\":\"${l}\"}",
                                                                  fc::mutable_variant_object()("l", salt))),
                                                                  abi_serializer_max_time);

      act_b_to_a.account = N(futio.token);
      act_b_to_a.name = NEX(transfer);
      act_b_to_a.authorization = vector<permission_level>{{name("user.112"),config::active_name}};
      act_b_to_a.data = futureio_token_serializer.variant_to_binary("transfer",
                                                                  fc::json::from_string(fc::format_string("{\"from\":\"user.112\",\"to\":\"user.111\",\"quantity\":\"0.1234 FGAS\",\"memo\":\"${l}\"}",
                                                                  fc::mutable_variant_object()("l", salt))),
                                                                  abi_serializer_max_time);
...
//            }
   ...
   void send_transaction(std::function<void(const fc::exception_ptr&)> next) {
      ...
      try {
          ...
          // public key: FUT7N6xm3NspBPPjFus3mewiaoN7iQtsJ142jM5p4qyCJgUze84VJ
         ...
         //         abi_serializer futureio_serializer(cc.db().find<account_object, by_name>(config::system_account_name)->get_abi());
...
//    abi_serializer futureio_token_serializer = fc::json::from_string(futureio_token_abi).as<abi_def>();
