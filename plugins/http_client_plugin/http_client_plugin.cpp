 ...
 */
#include <futureio/http_client_plugin/http_client_plugin.hpp>
            ...
            for( const auto& root_pem : root_pems ) {
                ...
                if( !boost::algorithm::starts_with( pem_str, "-----BEGIN CERTIFICATE-----\n" )) {
                    try {
                        ...
                        sstr << infile.rdbuf();
                        pem_str = sstr.str();
                        FUTURE_ASSERT( boost::algorithm::starts_with( pem_str, "-----BEGIN CERTIFICATE-----\n" ),
...
void http_client_plugin::plugin_startup() {
    chain_plugin* _chain_plugin = app().find_plugin<chain_plugin>();
    if (_chain_plugin) {
        chain::controller& cc = _chain_plugin->chain();
