...
BOOST_AUTO_TEST_CASE(publickey_checksum) {
        ...
        gm::sm2::PublicKey expectedPub("FUT7EL1bRiFqEfzcAUZBj3ggSLFd2UrkRwZkeC72tXWunia4panq9");
...
BOOST_AUTO_TEST_CASE(encrypt) {
        ...
        BOOST_CHECK(futureio::Hex::toHex<uint8_t>((uint8_t*)encoded.data(), encoded.size()) == std::string("681edf34d206965e86b3e94f536e4246"));

        // decrypt
        int blockSize = 16; // bytes
        uint8_t enc[blockSize];
        futureio::Hex::fromHex<uint8_t>("681edf34d206965e86b3e94f536e4246", enc, blockSize);
        ...
        BOOST_CHECK(futureio::Hex::toHex<uint8_t>((uint8_t*)result.data(), result.size()) == std::string("595298c7c6fd271f0402f804c33d3f66"));
}
...
BOOST_AUTO_TEST_CASE(EncDecPaddingGreateABlock) {
    ...
    futureio::Hex::fromHex<uint8_t>(keyStr, key, gm::sm4::Sm4::kCipherKeySize);
    int plainSize = 156;
    ...
    futureio::Hex::fromHex<uint8_t>(plainStr, plain, plainSize);

    std::vector<char> cipherActual = gm::sm4::Sm4::cbcEncrypt(key, plain, plainSize);
    std::cout << "cipher size : " << cipherActual.size() << " cipher: " << futureio::Hex::toHex<uint8_t>((uint8_t*)cipherActual.data(), cipherActual.size()) << std::endl;

    std::vector<char> plainActual = gm::sm4::Sm4::cbcDecrypt(key, (uint8_t*)cipherActual.data(), cipherActual.size());
        std::cout << "plain size : " << plainActual.size() << " plain: " << futureio::Hex::toHex<uint8_t>((uint8_t*)plainActual.data(), plainActual.size()) << std::endl;
    BOOST_CHECK(plainStr == futureio::Hex::toHex<uint8_t>((uint8_t*)plainActual.data(), plainActual.size()));
}
