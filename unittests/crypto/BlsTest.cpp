...
BOOST_AUTO_TEST_SUITE(bls_test_suite)
    BOOST_AUTO_TEST_CASE(aggragate) {
        ...
        blsPtr->sign(sk, (void*)future.c_str(), future.length(), sig, Bls::BLS_SIGNATURE_COMPRESSED_LENGTH);
        BOOST_CHECK(blsPtr->verify(pk, sig, (void*)future.c_str(), future.length()) == true);
        ...
        blsPtr->sign(sk1, (void*)future.c_str(), future.length(), sig1, Bls::BLS_SIGNATURE_COMPRESSED_LENGTH);
        BOOST_CHECK(blsPtr->verify(pk1, sig1, (void*)future.c_str(), future.length()) == true);
        ...
        blsPtr->sign(sk2, (void*)future.c_str(), future.length(), sig2, Bls::BLS_SIGNATURE_COMPRESSED_LENGTH);
        BOOST_CHECK(blsPtr->verify(pk2, sig2, (void*)future.c_str(), future.length()) == true);
        ...
        BOOST_CHECK(blsPtr->verifyAggregate(pks, 3, sigX, Bls::BLS_SIGNATURE_COMPRESSED_LENGTH, (void*)future.c_str(), future.length()));
    }
    ...
    BOOST_AUTO_TEST_CASE(diff_g) {
        ...
        std::shared_ptr<Bls> blsPtrNullG = std::make_shared<Bls>(Bls::FUTUREIO_BLS_DEFAULT_PARAM.c_str(), strlen(Bls::FUTUREIO_BLS_DEFAULT_PARAM.c_str()));
...
//        blsPtrNullG->keygen(skNullG, Bls::BLS_PRI_KEY_LENGTH, pkNullG, Bls::BLS_PUB_KEY_COMPRESSED_LENGTH);
        ...
        BOOST_CHECK(Hex::toHex<unsigned char>(nullG, Bls::BLS_G_LENGTH) != Bls::FUTUREIO_BLS_DEFAULT_G);
