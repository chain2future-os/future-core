...
docker ps | grep $NAME- | awk '{print $1}' | xargs -i docker exec -d {} bash -c "cp $FUTURE_PATH/scripts/config/config/{}.ini /root/.local/share/futureio/nodfuture/config/config.ini"
cmd="$FUTURE_PATH/scripts/_runfuture.sh "
...
cmd="nohup $FUTURE_PATH/scripts/logrotate.sh &"
...
docker ps | grep $NAME- | awk '{print $1}' | xargs -i docker exec -d {} bash -c "cp $FUTURE_PATH/scripts/ws/config.ini /root/.local/share/futureio/wssfuture/config/config.ini"
wscmd="$FUTURE_PATH/build/programs/wssfuture/wssfuture --http-server-address 127.0.0.1:7777 > /log/ws-\${HOSTNAME}.log 2>&1 &"
