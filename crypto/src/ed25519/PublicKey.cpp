...
namespace ed25519 {
    ...
    PublicKey::PublicKey(uint8_t* rawKey, size_t len) : m_key(futureio::Hex::toHex<uint8_t>(rawKey, len)) {}
    ...
    bool PublicKey::getRaw(uint8_t* rawKey, size_t len) const {
        return futureio::Hex::fromHex<uint8_t>(m_key, rawKey, len) == Ed25519::PUBLIC_KEY_LEN;
    }
