...
namespace ed25519 {
    ...
    PrivateKey::PrivateKey(uint8_t* rawKey, size_t len) : m_key(futureio::Hex::toHex<uint8_t>(rawKey, len)) {
    }
    ...
    bool PrivateKey::getRaw(uint8_t* rawKey, size_t len) const {
        return futureio::Hex::fromHex<uint8_t>(m_key, rawKey, len) == Ed25519::PRIVATE_KEY_LEN;
    }
