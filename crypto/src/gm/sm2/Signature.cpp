...
namespace gm {
    namespace sm2 {
        ...
        std::string Signature::bin2r(const Signature::DataType& bin) {
            return futureio::Hex::toHex<unsigned char>(bin.get<Signature::SignatureData>().begin() + 1, Signature::kRLength, false);
        }

        std::string Signature::bin2s(const Signature::DataType& bin) {
            return futureio::Hex::toHex<unsigned char>(bin.get<Signature::SignatureData>().begin() + 1 + Signature::kRLength, Signature::kSLength, false);
        }
        ...
        Signature::DataType Signature::hex2bin(const std::string& hexSig) {
            ...
            Signature::SignatureData data;
            data.begin()[0] = 33;
            futureio::Hex::fromHex<unsigned char>(hexSig, data.begin() + 1, Signature::kSignatureLength);
            return Signature::DataType(data);
        }
