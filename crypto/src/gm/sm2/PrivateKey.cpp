...
namespace gm {
    namespace sm2 {
        PrivateKey PrivateKey::generate() {
            ...
            privateKey.reset(PrivateKey::toWif(futureio::StringUtils::paddingPrefixZero(std::string(privateChar), PrivateKey::kSm2PrivateKeyLength * 2)));
        ...
        PrivateKey PrivateKey::regenerate(const fc::sha256& k) {
            std::string hexStr = futureio::Hex::toHex<unsigned char>((unsigned char*)k.data(), k.data_size(), false);
        ...
        bool PrivateKey::sign(const char* data, size_t dataSize, Signature& outSignature) const {
            ...
            finalR = futureio::StringUtils::paddingPrefixZero(std::string(sigRChar), Signature::kRLength * 2);
            finalS = futureio::StringUtils::paddingPrefixZero(std::string(sigSChar), Signature::kSLength * 2);
        ...
        std::string PrivateKey::toWif(const std::string& hexKey) {
            ...
            futureio::Hex::fromHex<unsigned char>(hexKey, rawKey, PrivateKey::kSm2PrivateKeyLength);
        ...
        std::string PrivateKey::fromWif(const std::string& wif) {
            ...
            return futureio::Hex::toHex<unsigned char>(rawKey, PrivateKey::kSm2PrivateKeyLength, false);
        }
