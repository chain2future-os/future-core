...
namespace futureio {
    ...
    void LightClient::accept(const BlockHeader& blockHeader, const std::string& signature) {
        ...
        if (m_workingCommitteeSet == CommitteeSet()) { // the first non genesis block
            FUTURE_ASSERT(m_nextCommitteeMroot != std::string(), chain::chain_exception, "m_nextCommitteeMroot is empty");
            FUTURE_ASSERT(CheckPoint::isCheckPoint(blockHeader), chain::chain_exception, "DO NOT pass check point when working committee set is empty");
            m_workingCommitteeSet = CheckPoint(blockHeader).committeeSet();
            FUTURE_ASSERT(std::string(m_workingCommitteeSet.committeeMroot()) == m_nextCommitteeMroot, chain::chain_exception, "working committee set' MRoot not equal ${root}", ("root", m_nextCommitteeMroot));
        }
    ...
    bool LightClient::isOutOfRange(const BlockHeader& blockHeader) const {
        FUTURE_ASSERT(m_latestConfirmedBlockId != BlockIdType(), chain::chain_exception, "latest confirmed block id is ${id}", ("id", BlockIdType()));
