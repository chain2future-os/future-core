...
namespace futureio {
    ...
    void LightClientProducer::handleConfirmPoint(futureio::chain::controller &chain, const BlsVoterSet& blsVoterSet) {
    ...
    void LightClientProducer::acceptNewHeader(const BlockHeader& blockHeader, const BlsVoterSet& blsVoterSet) {
        ...
        if (ConfirmPoint::isConfirmPoint(blockHeader)) {
            ...
            FUTURE_ASSERT(checkCanConfirm(confirmedBlockNum), chain::chain_exception, "check to confirm block : ${num}", ("num", confirmedBlockNum));
