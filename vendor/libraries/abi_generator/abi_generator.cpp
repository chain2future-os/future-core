#include <futureio/abi_generator/abi_generator.hpp>
...
string abi_generator::translate_type(const string& type_name) {
  ...
  if (type_name == "unsigned __int128" || type_name == "uint128_t") built_in_type = "uint128";
     ...
     static auto types = futureio::chain::common_type_defs();
     auto itr = std::find_if( types.begin(), types.end(),
                              [&type_name]( const futureio::chain::type_def& t ) { return t.new_type_name == type_name; } );
     if( itr != types.end()) {
...
bool abi_generator::inspect_type_methods_for_actions(const Decl* decl) { try {
  ...
  auto export_method = [&](const CXXMethodDecl* method) {
    ...
    // Check if current method is listed the FUTUREIO_ABI macro
...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // Check if the current declaration has actions (FUTUREIO_ABI, or explicit)
  ...
  // If FUTUREIO_ABI macro was found, we will only check if the current Decl
