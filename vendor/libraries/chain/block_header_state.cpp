...
namespace futureio { namespace chain {
  ...
  block_header_state block_header_state::generate_next( block_timestamp_type when )const {
    ...
    if( when != block_timestamp_type() ) {
       FUTURE_ASSERT( when > header.timestamp, block_validate_exception, "next block must be in the future" );
    } else {
    ...
    account_name producer_name = "futureio";
  ...
  block_header_state block_header_state::next( const signed_block_header& h, bool trust )const {
    FUTURE_ASSERT( h.timestamp != block_timestamp_type(), block_validate_exception, "", ("h",h) );
    //FUTURE_ASSERT( h.header_extensions.size() == 0, block_validate_exception, "no supported extensions" );

    FUTURE_ASSERT( h.timestamp > header.timestamp, block_validate_exception, "block must be later in time" );
    FUTURE_ASSERT( h.previous == id, unlinkable_block_exception, "block must link to current state" );
