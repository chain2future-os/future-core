...
#include <futureio/chain/block_summary_object.hpp>
...
#include <futureio/chain/transaction_object.hpp>
...
#include <futureio/chain/bls_votes.hpp>
#include <futureio/chain/chain_worldstate.hpp>
...
struct controller_impl {
    ...
    */
    ...
    /**
     * We import a new thread to do signature pre-verify and save it in cache.
     * The main thread fetch the public key from the cache when do transaction signature verify
     */
    std::atomic_bool done{false};

    bool is_non_producer = false;

    bool thread_startup_flag = false;

    boost::thread sig_verify_thread;

    typedef struct pending_trx_backup_buffer {
       std::atomic_int next_index{0};
       const int capacity = 10000;
       std::vector<transaction_metadata_ptr> ring_buffer;
       pending_trx_backup_buffer() {
           ring_buffer.reserve(capacity);
       }

       void push_back(const transaction_metadata_ptr& trx) {
           if (ring_buffer.size() < capacity) {
               ring_buffer.push_back(trx);
           } else {
               ring_buffer[next_index] = trx;
           }
           next_index = (++next_index) % capacity;
       }

       int head() const {
           if (ring_buffer.size() >= capacity && next_index == 0) {
               return capacity - 1;
           }
           return next_index - 1;
       }
       ...
       transaction_metadata_ptr get(size_t index) {
          // main thread may write ring_buffer[next_index] when index == next_index
          if (index >= ring_buffer.size() || index == next_index) {
             return nullptr;
          }
          return ring_buffer[index];
       }
    } pending_trx_backup_buffer_type;

    pending_trx_backup_buffer_type pending_trx_backup;

    typedef struct pending_trx_sig_to_pubkey_cache {
        static constexpr int buffer_number = 3;
        const int buffer_size = 15000;
        sig_to_pubkey_map_type* map_arr[buffer_number] = {nullptr};
        boost::mutex* mutex_arr[buffer_number];
        std::atomic_int working_index{0};

        pending_trx_sig_to_pubkey_cache() {
            for (int i = 0; i < buffer_number; i++) {
                map_arr[i] = new sig_to_pubkey_map_type();
                mutex_arr[i] = new boost::mutex();
            }
        }
        ...
        bool get(const signature_type& sig, const transaction_id_type& id, public_key_type& out_key) {
            int search_index = working_index;
            search_index = ++search_index % buffer_number; // search from the oldest buffer
            for (int i = 0; i < buffer_number; i++) {
                boost::lock_guard<boost::mutex> lock(*mutex_arr[search_index]);
                sig_to_pubkey_map_type* search_map_ptr = map_arr[search_index];
                auto itr = search_map_ptr->find(sig);
                if (itr != search_map_ptr->end() && itr->second.second == id) {
                    out_key = itr->second.first;
                    return true;
                }
                search_index = ++search_index % buffer_number;
            }
            return false;
        }

        flat_set<public_key_type> get(const std::vector<signature_type> sigs, const transaction_id_type& id) {
        ...
        void add(const sig_to_pubkey_map_type& map) {
            {
                boost::lock_guard<boost::mutex> lock(*mutex_arr[working_index]);
                for (auto itr = map.begin(); itr != map.end(); itr++) {
                    sig_to_pubkey_map_type* map = map_arr[working_index];
                    (*map)[itr->first] = itr->second;
                }
            }
            if (map_arr[working_index]->size() > buffer_size) {
                working_index = ++working_index % buffer_number;
                ilog("clear back buffer, index : ${i}", ("i", int(working_index)));
                boost::lock_guard<boost::mutex> lock(*mutex_arr[working_index]);
                map_arr[working_index]->clear();
            }
        }

    } pending_trx_sig_to_pubkey_cache_type;

    pending_trx_sig_to_pubkey_cache_type pending_trx_sig_to_pubkey_cache;
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( futureio, futureio, newaccount );
   SET_APP_HANDLER( futureio, futureio, setcode );
   SET_APP_HANDLER( futureio, futureio, setabi );
   SET_APP_HANDLER( futureio, futureio, updateauth );
   SET_APP_HANDLER( futureio, futureio, deleteauth );
   SET_APP_HANDLER( futureio, futureio, linkauth );
   SET_APP_HANDLER( futureio, futureio, unlinkauth );
   SET_APP_HANDLER( futureio, futureio, delaccount );

/*
   SET_APP_HANDLER( futureio, futureio, postrecovery );
   SET_APP_HANDLER( futureio, futureio, passrecovery );
   SET_APP_HANDLER( futureio, futureio, vetorecovery );
*/

   SET_APP_HANDLER( futureio, futureio, canceldelay );
   SET_APP_HANDLER( futureio, futureio, addwhiteblack );
   SET_APP_HANDLER( futureio, futureio, rmwhiteblack );
   ...
   void verify_sig_thread_func() {
      ...
      while (true) {
          ...
          if (pending_trx_backup.head() >= 0 && head != pending_trx_backup.head()) {
              try {
                  head = ++head % pending_trx_backup.cap();
                  transaction_metadata_ptr trx = pending_trx_backup.get(head);
                  FUTURE_ASSERT(trx, chain_exception, "nullptr trx");
#ifdef FUTURE_TRX_SUPPORT_GM
                  //restore_keys( trx, pending_trx_sig_to_pubkey_cache.get_working_buffer() );
#else
                  sig_to_pubkey_map_type sig_to_pk = trx->recover_keys_fresh(chain_id, nullptr);
                  pending_trx_sig_to_pubkey_cache.add(sig_to_pk);
#endif
              } catch (const fc::exception &e) {
                  elog("pre verify sig ex happend : ${i}", ("i", e.to_detail_string()));
              } catch (...) {
                  elog("unknown exception happen");
              }
          } else {
   ...
   void on_irreversible( const block_state_ptr& s ) {
      ...
      if (!log_head) {
         if (s->block) {
            ...
            FUTURE_ASSERT(s->block_num == blog.first_block_num() - 1, block_log_exception, "block log has no blocks and is not properly set up to start after the worldstate");
         }
      } else {
         auto lh_block_num = log_head->block_num();
         if (s->block_num > lh_block_num) {
            FUTURE_ASSERT(s->block_num - 1 == lh_block_num, unlinkable_block_exception, "unlinkable block", ("s->block_num", s->block_num)("lh_block_num", lh_block_num));
            FUTURE_ASSERT(s->block->previous == log_head->id(), unlinkable_block_exception, "irreversible doesn't link to block log head");
            append_to_blog = true;
         }
      }
      ...
      if (s->block) {
         if (read_mode == db_read_mode::IRREVERSIBLE) {
            ...
            if (!head || s->block_num == head->block_num + 1) {
               ...
               FUTURE_ASSERT(!head || head->block_num == 1, block_validate_exception, "Attempting to re-apply an irreversible block that was not the implied genesis block");
            }
   ...
   void init(const bfs::path& worldstate_path) {
      ...
      */
      if (bfs::exists(worldstate_path)) {
         FUTURE_ASSERT(!head, fork_database_exception, "");
         ...
         if( !end ) {
            ...
            FUTURE_ASSERT(end->block_num() == head->block_num, fork_database_exception,
      ...
      FUTURE_ASSERT( db.revision() >= head->block_num, fork_database_exception, "fork database is inconsistent with shared memory",
   ...
   ~controller_impl() {
      ...
      if (sig_verify_thread.joinable()) {
          ilog("Sig verify thread is running, waiting!!");
          sig_verify_thread.join();
      }
   }
   ...
   bool restore_contract_database_index(chainbase::database& worldstate_db, worldstate_reader::section_reader& section_reader,
      ...
      contract_database_index_set::walk_indices([&](auto utils) {
         ...
         for (size_t idx = 0; idx < size.value; idx++) {
            ...
            FUTURE_ASSERT(more == id_more, worldstate_exception, "Restore to backup indices error: the ws data conflict ${x} ${t}", ("x", more)("t", id_more));
         }
      });
   ...
   void store_one_contract_table(chainbase::database& worldstate_db, worldstate_writer::section_writer& writer_section, std::shared_ptr<ws_helper> ws_helper_ptr){
      ...
      FUTURE_ASSERT(count <= 1, worldstate_exception, "The table_id count should not more than 1, because we handle only one every time!!");
   }

   void add_contract_tables_to_worldstate(std::shared_ptr<ws_helper> ws_helper_ptr, chainbase::database& worldstate_db) {
      FUTURE_ASSERT(ws_helper_ptr->get_writer() && ws_helper_ptr->get_id_writer(), worldstate_exception, "Ws writer is not exist!");
      ...
      ws_helper_ptr->get_writer()->write_section("contract_tables", [&]( auto& writer_section ){
         if(ws_helper_ptr->get_id_reader() && ws_helper_ptr->get_reader()) {
            ...
            ws_helper_ptr->get_reader()->read_section("contract_tables", [&]( auto& reader_section ) {
               ...
               FUTURE_ASSERT(more == id_more, worldstate_exception, "Restore to backup indices error: the ws data conflict ");
   ...
   void read_contract_tables_from_worldstate( std::shared_ptr<ws_helper> ws_helper_ptr, chainbase::database& worldstate_db ) {
      FUTURE_ASSERT(ws_helper_ptr->get_reader(), worldstate_exception, "Ws reaer is not exist!");
   ...
   void add_to_worldstate(chainbase::database& worldstate_db, uint32_t block_height, const block_state& fork_head){
      ...
      if (worldstate_previous_block > 0) {
         FUTURE_ASSERT(fc::exists(old_ws_file + ".ws") && fc::exists(old_ws_file + ".id"), worldstate_exception, "Don't exist file: ${s} or ${t}",
   ...
   void read_from_worldstate( const bfs::path& worldstate_path ) {
      ...
      FUTURE_ASSERT(ws_manager_ptr, worldstate_exception, "ws_manager_ptr is null!");
   ...
   void create_native_account( account_name name, const authority& owner, const authority& active, bool is_privileged = false, bool is_updateable = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            a.set_abi(futureio_contract_abi(abi_def()));
         }
      });
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         if (add_to_fork_db) {
            ...
            head = fork_db.head();
            FUTURE_ASSERT(new_bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      FUTURE_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      FUTURE_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      FUTURE_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      FUTURE_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         try {
            ...
            flat_set<public_key_type> pks;
#else
            flat_set<public_key_type> pks = pending_trx_sig_to_pubkey_cache.get( trx->trx.signatures, trx->trx.id() );
            if (pks.size() != trx->trx.signatures.size()) { // not hit the cache
                pks.clear();
                sig_to_pubkey_map_type map = trx->recover_keys_fresh(chain_id, &pks);
                if (is_non_producer || !thread_startup_flag) {
                    pending_trx_sig_to_pubkey_cache.add(map);
                }
            }
#endif
    ...
    void start_block( block_timestamp_type when, chain::checksum256_type committee_mroot, std::string sig, controller::block_status s ) {
      FUTURE_ASSERT( !pending, block_validate_exception, "pending block is not available" );

      FUTURE_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
   ...
   void assign_header_to_block() {
    ...
    void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
      try {
         FUTURE_ASSERT( b->block_extensions.size() == 0, block_validate_exception, "no supported extensions" );
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               FUTURE_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
            ...
            FUTURE_ASSERT( pending->_pending_block_state->block->transactions.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            FUTURE_ASSERT( pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
            ...
            FUTURE_ASSERT( r == static_cast<const transaction_receipt_header&>(receipt),
         ...
         FUTURE_ASSERT(b->id() == pending->_pending_block_state->header.id(),
   ...
   void push_block( const signed_block_ptr& b, controller::block_status s ) {
    //  idump((fc::json::to_pretty_string(*b)));
      FUTURE_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
      try {
         FUTURE_ASSERT( b, block_validate_exception, "trying to push empty block" );
         FUTURE_ASSERT( s != controller::block_status::incomplete, block_validate_exception, "invalid block status for a completed block" );
   ...
   void register_event(const std::string& account, const std::string& post_url) {
      FUTURE_ASSERT(can_accept_event_register, event_register_wrong_node, "Can't register event on producing node.");
      ...
      else
      {
        ...
        {
           FUTURE_ASSERT(std::string(*list_it) != post_url, event_register_duplicate, "Duplicate register.");
        }
   ...
   void unregister_event(const std::string& account, const std::string& post_url) {
      ...
      FUTURE_ASSERT(false, event_unregister_error, "Wrong account or unregistered url.");
   }
   ...
   void push_event(account_name act_name, transaction_id_type id, const char* event_name, size_t event_name_size,
      ...
      ilog("act name: ${an} trx: ${trx} event name: ${en} msg: ${msg}", ("an", act_name)("trx", id)("en", ename)("msg", msg));
   }
   ...
   void maybe_switch_forks( controller::block_status s = controller::block_status::complete ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         FUTURE_ASSERT(false, chain_exception, "should never switch forks");
      }
   } /// push_block
   ...
   void finalize_block()
   {
      FUTURE_ASSERT(pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      try {
      ...
      */
      ...
      uint64_t CPU_TARGET = FUTURE_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      ...
      resource_limits.set_block_parameters(
         ...
         {FUTURE_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, max_virtual_mult, {99, 100}, {1000, 999}}
      );
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      if( whiteblack_list.actor_whitelist.size() > 0 ) {
         ...
         if( excluded.size() == 1 && excluded[0] == config::system_account_name )
             return ;
         FUTURE_ASSERT( excluded.size() == 0, actor_whitelist_exception,
         ...
         FUTURE_ASSERT( blacklisted.size() == 0, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      ...
      if( whiteblack_list.contract_whitelist.size() > 0 ) {
         FUTURE_ASSERT( whiteblack_list.contract_whitelist.find( code ) != whiteblack_list.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( whiteblack_list.contract_blacklist.size() > 0 ) {
         FUTURE_ASSERT( whiteblack_list.contract_blacklist.find( code ) == whiteblack_list.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( whiteblack_list.action_blacklist.size() > 0 ) {
         FUTURE_ASSERT( whiteblack_list.action_blacklist.find( std::make_pair(code, action) ) == whiteblack_list.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( whiteblack_list.key_blacklist.size() > 0 ) {
         FUTURE_ASSERT( whiteblack_list.key_blacklist.find( key ) == whiteblack_list.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      FUTURE_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
time_point controller::pending_block_time()const {
   FUTURE_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   FUTURE_ASSERT( BOOST_LIKELY( signed_blk != nullptr ), unknown_block_exception,
...
vector<transaction_metadata_ptr> controller::get_unapplied_transactions() {
   ...
   if ( my->read_mode == db_read_mode::SPECULATIVE ) {
      ...
      FUTURE_ASSERT( my->unapplied_transactions.empty(), transaction_exception, "not empty unapplied_transactions in non-speculative mode" ); //should never happen
   }
   return result;
}
...
std::pair<bool, bool> controller::push_into_pending_transaction(const transaction_metadata_ptr& trx) {
    if (my->pending_transactions.size() > futureio::chain::config::default_max_pending_trx_count ||
        my->unapplied_transactions.size() > futureio::chain::config::default_max_unapplied_trx_count)
        return std::pair<bool, bool>(true, false);

    if (my->pending_transactions_set.find(trx->signed_id) == my->pending_transactions_set.end()) {
        if (my->thread_startup_flag) {
            my->pending_trx_backup.push_back(std::make_shared<transaction_metadata>(trx->trx));
        }
    ...
    FUTURE_ASSERT( my->pending_transactions_set.size() == my->pending_transactions.size(),
                    chain_exception,
...
void controller::startup_sig_verify_thread(bool is_non_producer, bool flag) {
    my->is_non_producer = is_non_producer;
    my->thread_startup_flag = flag;

    // startup thread only for producer node and the flag on
    if (!is_non_producer && flag) {
        my->sig_verify_thread = boost::thread([this] { my->verify_sig_thread_func(); } );
    }
}
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   FUTURE_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   FUTURE_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   FUTURE_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
