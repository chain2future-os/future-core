...
void transaction_header::validate()const {
   FUTURE_ASSERT( max_net_usage_words.value < UINT32_MAX / 8UL, transaction_exception,
...
{ try {
   ...
   FUTURE_ASSERT(false, tx_decompression_error, "FUTURE_TRX_SUPPORT_GM do not support, should not be here");
#else
   for(const signature_type& sig : signatures) {
      ...
      FUTURE_ASSERT( allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
sig_to_pubkey_map_type transaction::recover_keys_fresh(const vector<signature_type>& signatures, const chain_id_type& chain_id,
        const vector<bytes>& cfd, flat_set<public_key_type>* pks)
{ try {
   sig_to_pubkey_map_type map;
   const digest_type digest = sig_digest(chain_id, cfd);
   for (const signature_type& sig : signatures) {
       public_key_type key = public_key_type(sig, digest);
       map[sig] = std::make_pair(key, id());
       if (pks != nullptr) {
           bool successful_insertion = false;
           std::tie(std::ignore, successful_insertion) = pks->insert(key);
           FUTURE_ASSERT( successful_insertion, tx_duplicate_sig,
                           "transaction includes more than one signature signed using the same key associated with public key: ${key}",
                           ("key", key)
           );
       }
...
sig_to_pubkey_map_type signed_transaction::recover_keys_fresh(const chain_id_type& chain_id, flat_set<public_key_type>* pks) {
    return transaction::recover_keys_fresh(signatures, chain_id, context_free_data, pks);
}
...
uint32_t packed_transaction::get_unprunable_size()const {
   ...
   FUTURE_ASSERT( size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big" );
...
struct read_limiter {
   ...
   {
      FUTURE_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
      _total += count;
...
bytes packed_transaction::get_raw_transaction() const
{
   try {
      switch(compression) {
         ...
         default:
            FUTURE_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
      }
