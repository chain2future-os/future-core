...
#include <futureio/chain/name_ex.hpp>
...
void name_ex::set(const char* str) {
    ...
    FUTURE_ASSERT(len <= 21, name_ex_type_exception, "NameEx is longer than 22 characters (${name}) ",
            ("name", string(str)));
    name_ex t = string_to_name_ex(str);
    valueH = t.valueH;
    valueL = t.valueL;

    FUTURE_ASSERT(to_string() == string(str), name_ex_type_exception,
