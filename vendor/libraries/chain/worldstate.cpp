...
void variant_worldstate_reader::validate() const {
   FUTURE_ASSERT(worldstate.is_object(), worldstate_validation_exception,
         "Variant worldstate is not an object");
   const fc::variant_object& o = worldstate.get_object();

   FUTURE_ASSERT(o.contains("version"), worldstate_validation_exception,
         "Variant worldstate has no version");

   const auto& version = o["version"];
   FUTURE_ASSERT(version.is_integer(), worldstate_validation_exception,
         "Variant worldstate version is not an integer");

   FUTURE_ASSERT(version.as_uint64() == (uint64_t)current_worldstate_version, worldstate_validation_exception,
   ...
   FUTURE_ASSERT(o.contains("sections"), worldstate_validation_exception,
         "Variant worldstate has no sections");

   const auto& sections = o["sections"];
   FUTURE_ASSERT(sections.is_array(), worldstate_validation_exception, "Variant worldstate sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      FUTURE_ASSERT(section.is_object(), worldstate_validation_exception, "Variant worldstate section is not an object");

      const auto& so = section.get_object();
      FUTURE_ASSERT(so.contains("name"), worldstate_validation_exception,
            "Variant worldstate section has no name");

      FUTURE_ASSERT(so["name"].is_string(), worldstate_validation_exception,
                 "Variant worldstate section name is not a string");

      FUTURE_ASSERT(so.contains("rows"), worldstate_validation_exception,
                 "Variant worldstate section has no rows");

      FUTURE_ASSERT(so["rows"].is_array(), worldstate_validation_exception,
...
void variant_worldstate_reader::set_section( const string& section_name ) {
   ...
   FUTURE_THROW(worldstate_exception, "Variant worldstate has no section named ${n}", ("n", section_name));
}
...
{
   FUTURE_ASSERT(section_pos == std::streampos(-1), worldstate_exception, "Attempting to write a new section without closing the previous section");
...
void istream_worldstate_reader::validate() const {
   ...
   try {
      ...
      FUTURE_ASSERT(actual_totem == expected_totem, worldstate_exception,
      ...
      FUTURE_ASSERT(actual_version == expected_version, worldstate_exception,
...
bool istream_worldstate_reader::get_section_info(uint64_t& section_size, uint64_t& row_count, int& data_pos, const std::string& section_name)
{
   ...
   return false;
   // FUTURE_THROW(worldstate_exception, "Binary worldstate has no section named ${n}", ("n", section_name));
}
...
bool istream_worldstate_reader::read_row(uint64_t size, std::vector<char>& out_data)
{
   try {
      ...
      if (read_cnt != size){
         FUTURE_THROW(worldstate_exception, "Binary worldstate read section error, \
      ...
      FUTURE_THROW(worldstate_exception, "Binary worldstate read section error. size ${n}", ("n", size));
   }
   return false;
}
...
void istream_worldstate_id_reader::read_start_id_section( const string& section_name ) {
   ...
   FUTURE_THROW(worldstate_exception, "Binary worldstate_id_ostream has no section named ${n}", ("n", section_name));
}
