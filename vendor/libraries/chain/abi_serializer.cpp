...
namespace futureio { namespace chain {
   ...
   void abi_serializer::set_abi(const abi_def& abi, const fc::microseconds& max_serialization_time) {
      ...
      for( const auto& td : abi.types ) {
         FUTURE_ASSERT(_is_type(td.type, 0, deadline, max_serialization_time), invalid_type_inside_abi, "invalid type", ("type",td.type));
         FUTURE_ASSERT(!_is_type(td.new_type_name, 0, deadline, max_serialization_time), duplicate_abi_type_def_exception, "type already exists", ("new_type_name",td.new_type_name));
       ...
       *  an invalid ABI
       */
      FUTURE_ASSERT( typedefs.size() == abi.types.size(), duplicate_abi_type_def_exception, "duplicate type definition detected" );
      FUTURE_ASSERT( structs.size() == abi.structs.size(), duplicate_abi_struct_def_exception, "duplicate struct definition detected" );
      FUTURE_ASSERT( actions.size() == abi.actions.size(), duplicate_abi_action_def_exception, "duplicate action definition detected" );
      FUTURE_ASSERT( tables.size() == abi.tables.size(), duplicate_abi_table_def_exception, "duplicate table definition detected" );
      FUTURE_ASSERT( error_messages.size() == abi.error_messages.size(), duplicate_abi_err_msg_def_exception, "duplicate error message definition detected" );
   ...
   int abi_serializer::get_integer_size(const type_name& type) const {
      string stype = type;
      FUTURE_ASSERT( is_integer(type), invalid_type_inside_abi, "${stype} is not an integer type", ("stype",stype));
   ...
   bool abi_serializer::_is_type(const type_name& rtype, size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time)const {
      FUTURE_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
   ...
   const struct_def& abi_serializer::get_struct(const type_name& type)const {
      ...
      FUTURE_ASSERT( itr != structs.end(), invalid_type_inside_abi, "Unknown struct ${type}", ("type",type) );
      return itr->second;
   }
   ...
   void abi_serializer::validate(const fc::time_point& deadline, const fc::microseconds& max_serialization_time)const {
      for( const auto& t : typedefs ) { try {
         ...
         while( itr != typedefs.end() ) {
            FUTURE_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
            FUTURE_ASSERT( find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(), abi_circular_def_exception, "Circular reference in type ${type}", ("type",t.first) );
            types_seen.emplace_back(itr->second);
            itr = typedefs.find(itr->second);
         }
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& t : typedefs ) { try {
         FUTURE_ASSERT(_is_type(t.second, 0, deadline, max_serialization_time), invalid_type_inside_abi, "", ("type",t.second) );
      } FC_CAPTURE_AND_RETHROW( (t) ) }
      for( const auto& s : structs ) { try {
         if( s.second.base != type_name() ) {
            ...
            while( current.base != type_name() ) {
               ...
               FUTURE_ASSERT( find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(), abi_circular_def_exception, "Circular reference in struct ${type}", ("type",s.second.name) );
            ...
            FUTURE_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
            FUTURE_ASSERT(_is_type(field.type, 0, deadline, max_serialization_time), invalid_type_inside_abi, "", ("type",field.type) );
          } FC_CAPTURE_AND_RETHROW( (field) ) }
      } FC_CAPTURE_AND_RETHROW( (s) ) }
      for( const auto& a : actions ) { try {
        FUTURE_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
        FUTURE_ASSERT(_is_type(a.type, 0, deadline, max_serialization_time), invalid_type_inside_abi, "", ("type",a.type) );
   ...
   {
      FUTURE_ASSERT( ++recursion_depth < max_recursion_depth, abi_recursion_depth_exception, "recursive definition, max_recursion_depth ${r} ", ("r", max_recursion_depth) );
      FUTURE_ASSERT( fc::time_point::now() < deadline, abi_serialization_deadline_exception, "serialization time limit ${t}us exceeded", ("t", max_serialization_time) );
                                                   ...
                                                   size_t recursion_depth, const fc::time_point& deadline, const fc::microseconds& max_serialization_time )const
   {
      ...
      if ( is_map(rtype)) {
          ...
          FUTURE_ASSERT(v.size() == 2, invalid_type_inside_abi, "Invalid packed map" );
        ...
        for( decltype(size.value) i = 0; i < size; ++i ) {
           ...
           FUTURE_ASSERT( !v.is_null(), unpack_exception, "Invalid packed array" );
           vars.emplace_back(std::move(v));
        }
        FUTURE_ASSERT( vars.size() == size.value,
                    unpack_exception,
      ...
      FUTURE_ASSERT( mvo.size() > 0, unpack_exception, "Unable to unpack stream ${type}", ("type", type) );
         ...
         if( var.is_object() ) {
            ...
            for( const auto& field : st.fields ) {
               if( vo.contains( string(field.name).c_str() ) ) {
                  ...
                  FUTURE_THROW( pack_exception, "Missing '${f}' in variant object", ("f",field.name) );
               }
            }
         } else if( var.is_array() ) {
            const auto& va = var.get_array();
            FUTURE_ASSERT( st.base == type_name(), invalid_type_inside_abi, "support for base class as array not yet implemented" );
            uint32_t i = 0;
            if (va.size() > 0) {
               for( const auto& field : st.fields ) {
                  if( va.size() > i )
