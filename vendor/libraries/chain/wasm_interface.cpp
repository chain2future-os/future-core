...
#include <futureio/chain/callback_manager.hpp>
#include <boost/core/ignore_unused.hpp>
#include <futureio/chain/authorization_manager.hpp>
#include <futureio/chain/resource_limits.hpp>
#include <futureio/chain/wasm_interface_private.hpp>
#include <futureio/chain/wasm_futureio_validation.hpp>
#include <futureio/chain/wasm_futureio_injection.hpp>
...
#include <futureio/chain/webassembly/wabt.hpp>
#include <futureio/chain/webassembly/binaryen.hpp>
...
namespace futureio { namespace chain {
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      try {
         ...
         FUTURE_ASSERT(false, wasm_serialization_error, e.message.c_str());
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.context_free )
            FUTURE_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
...
#ifdef FUTURE_SUPPORT_TYPESCRIPT
class typescript_action_api : public context_aware_api {
   ...
   public:
    ...
    void future_assert_native(int condition) {
        FC_ASSERT(condition != 0, "future_assert");
   }
    /*
...
class typescript_crypto_api : public context_aware_api {
   public:
...
#ifdef FUTURE_TRX_SUPPORT_GM
         bool verified = p.verify(digest.data(), digest.data_size(), s);
         FUTURE_ASSERT( verified == true, crypto_api_exception, "Error expected key different than recovered key" );
#else
         auto check = public_key_type( s, digest, false );
         FUTURE_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
#endif
      }
      ...
      int ts_recover_key( array_ptr<char> hash, size_t hashlen,
         ...
         FUTURE_ASSERT(false, crypto_api_exception, "FUTURE_TRX_SUPPORT_GM not provide ts_recover_key");
         return 0;
#else
         signature_type s;
         datastream<const char*> ds( sig, siglen );
      ...
      template<class Encoder> auto encode(char* data, size_t datalen) {
         Encoder e;
         const size_t bs = futureio::chain::config::hashing_checktime_block_size;
         while ( datalen > bs ) {
            e.write( data, bs );
            data += bs;
            datalen -= bs;
      ...
      void ts_assert_sha256(array_ptr<char> data, size_t datalen, array_ptr<char> hash, size_t hashlen) {
         ...
         FUTURE_ASSERT( result == hash_val, crypto_api_exception, "hash as sha256 mismatch" );
      }
      ...
      void ts_assert_sha1(array_ptr<char> data, size_t datalen, array_ptr<char> hash, size_t hashlen) {
         ...
         FUTURE_ASSERT( result == hash_val, crypto_api_exception, "hash as sha1 mismatch" );
      }
      ...
      void ts_assert_sha512(array_ptr<char> data, size_t datalen, array_ptr<char> hash, size_t hashlen) {
         ...
         FUTURE_ASSERT( result == hash_val, crypto_api_exception, "hash as sha512 mismatch" );
      }
      ...
      void ts_assert_ripemd160(array_ptr<char> data, size_t datalen, array_ptr<char> hash, size_t hashlen) {
         ...
         FUTURE_ASSERT( result == hash_val, crypto_api_exception, "hash as ripemd160 mismatch" );
      }
      ...
      int ts_verify_with_pk(null_terminated_ptr pk_str, null_terminated_ptr pk_proof, null_terminated_ptr message) {
         return futureio::verify_with_pk(pk_str.value, pk_proof.value, message.value) ? 1 : 0;
      }
      ...
      int ts_check_shielded_transaction(array_ptr<char> trx_bytes, size_t len) {
          ShieldedTrxPack trx;
          datastream<char *> ds(trx_bytes, len);
          fc::raw::unpack(ds, trx);
          if (libsnark::check_shielded_transaction(trx.toShieldedTrx())) {
              return 0;
          } else {
              return -1;
          }
      ...
      int ts_update_inc_merkle_tree(array_ptr<char> merkle_bytes, size_t merkle_len, uint32_t buffer_len,
                                    array_ptr<char> cm_bytes, size_t cm_len,
                                    array_ptr<char> anchor_bytes, size_t anchor_len,
                                    array_ptr<char> pos_bytes, size_t pos_len) {
          if (merkle_len == 0 || cm_len == 0) return -1;

          libzcash::MerkleTreePack mpack;
          datastream<const char*> tree_ds(merkle_bytes, merkle_len);
          fc::raw::unpack(tree_ds, mpack);
          SaplingMerkleTree tree(mpack);
          if (!tree.wfcheck()) {
              return -1;
          }

          vector<string> strcms;
          datastream<const char*> cm_ds(cm_bytes, cm_len);
          fc::raw::unpack(cm_ds, strcms);

          size_t per_anchor_len = anchor_len / strcms.size();

          for (size_t i = 0; i < strcms.size(); i++) {
              zero::uint256 cm;
              cm.SetHex(strcms[i]);
              tree.append(cm);

              std::string anchor_str = tree.root().GetHex();
              if (anchor_str.length() != per_anchor_len) {
                  return -1;
              }
              memcpy((char*)anchor_bytes+i*per_anchor_len, anchor_str.c_str(), per_anchor_len);

              auto wit = tree.witness();
              uint64_t pos = wit.position();
              memcpy((char*)pos_bytes+i*sizeof(uint64_t), &pos, sizeof(uint64_t));
          }

          libzcash::MerkleTreePack new_mpack = tree.toMerkleTreePack();
          int32_t length = fc::raw::pack_size(new_mpack);
          if (length <= buffer_len) {
              datastream<char*> new_tree_ds(merkle_bytes, buffer_len);
              fc::raw::pack(new_tree_ds, new_mpack.left);
              fc::raw::pack(new_tree_ds, new_mpack.right);
              fc::raw::pack(new_tree_ds, new_mpack.parents);
          } // else, the buffer is not enough, and we also return the needed length.
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         FUTURE_ASSERT( context.context_free, unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         FUTURE_ASSERT( context.privileged, unaccessible_api, "${code} does not have permission to call this API", ("code",context.receiver) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         FUTURE_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         FUTURE_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         FUTURE_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         FUTURE_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      void get_account_pubkey(account_name user, array_ptr<char> owner_pub, size_t owner_publen, array_ptr<char> active_pub, size_t active_publen ) {
         ...
         FUTURE_ASSERT( permission_o.auth.keys.size() > 0, crypto_api_exception, " account not exist permission ower public key" );
         ...
         FUTURE_ASSERT( permission_a.auth.keys.size() > 0, crypto_api_exception, " account not exist permission active public key" );
...
class softfloat_api : public context_aware_api {
   public:
      ...
      float _futureio_f32_max( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a)) {
            return af;
      ...
      float _futureio_f32_copysign( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
      ...
      // float unops
      float _futureio_f32_abs( float af ) {
         float32_t a = to_softfloat32(af);
         a.v &= ~(1 << 31);
         return from_softfloat32(a);
      }
      float _futureio_f32_neg( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _futureio_f32_floor( float af ) {
         float32_t a = to_softfloat32(af);
         int e = (int)(a.v >> 23 & 0xFF) - 0X7F;
         uint32_t m;
         if (e >= 23)
      ...
      float _futureio_f32_trunc( float af ) {
         float32_t a = to_softfloat32(af);
      ...
      float _futureio_f32_nearest( float af ) {
         float32_t a = to_softfloat32(af);
         int e = a.v>>23 & 0xff;
         int s = a.v>>31;
         float32_t y;
      ...
      // float relops
      bool _futureio_f32_eq( float a, float b ) {  return f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _futureio_f32_ne( float a, float b ) { return !f32_eq( to_softfloat32(a), to_softfloat32(b) ); }
      bool _futureio_f32_lt( float a, float b ) { return f32_lt( to_softfloat32(a), to_softfloat32(b) ); }
      bool _futureio_f32_le( float a, float b ) { return f32_le( to_softfloat32(a), to_softfloat32(b) ); }
      bool _futureio_f32_gt( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      bool _futureio_f32_ge( float af, float bf ) {
         float32_t a = to_softfloat32(af);
         float32_t b = to_softfloat32(bf);
         if (is_nan(a))
            return false;
      ...
      double _futureio_f64_max( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return af;
      ...
      double _futureio_f64_copysign( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
      ...
      // double unops
      double _futureio_f64_abs( double af ) {
         float64_t a = to_softfloat64(af);
         a.v &= ~(uint64_t(1) << 63);
         return from_softfloat64(a);
      }
      double _futureio_f64_neg( double af ) {
         float64_t a = to_softfloat64(af);
      ...
      double _futureio_f64_sqrt( double a ) {
         float64_t ret = f64_sqrt( to_softfloat64(a) );
         return from_softfloat64(ret);
      }
      // ceil, floor, trunc and nearest are lifted from libc
      double _futureio_f64_ceil( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7ff;
         float64_t y;
      ...
      double _futureio_f64_floor( double af ) {
         float64_t a = to_softfloat64( af );
         float64_t ret;
         int e = a.v >> 52 & 0x7FF;
         float64_t y;
      ...
      double _futureio_f64_trunc( double af ) {
         float64_t a = to_softfloat64( af );
      ...
      double _futureio_f64_nearest( double af ) {
         float64_t a = to_softfloat64( af );
         int e = (a.v >> 52 & 0x7FF);
         int s = a.v >> 63;
         float64_t y;
      ...
      // double relops
      bool _futureio_f64_eq( double a, double b ) { return f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _futureio_f64_ne( double a, double b ) { return !f64_eq( to_softfloat64(a), to_softfloat64(b) ); }
      bool _futureio_f64_lt( double a, double b ) { return f64_lt( to_softfloat64(a), to_softfloat64(b) ); }
      bool _futureio_f64_le( double a, double b ) { return f64_le( to_softfloat64(a), to_softfloat64(b) ); }
      bool _futureio_f64_gt( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      bool _futureio_f64_ge( double af, double bf ) {
         float64_t a = to_softfloat64(af);
         float64_t b = to_softfloat64(bf);
         if (is_nan(a))
            return false;
      ...
      double _futureio_f32_promote( float a ) {
         return from_softfloat64(f32_to_f64( to_softfloat32(a)) );
      }
      float _futureio_f64_demote( double a ) {
         return from_softfloat32(f64_to_f32( to_softfloat64(a)) );
      }
      int32_t _futureio_f32_trunc_i32s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_futureio_f32_ge(af, 2147483648.0f) || _futureio_f32_lt(af, -2147483648.0f))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_s/i32 overflow" );

         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_s/i32 unrepresentable");
         return f32_to_i32( to_softfloat32(_futureio_f32_trunc( af )), 0, false );
      }
      int32_t _futureio_f64_trunc_i32s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_futureio_f64_ge(af, 2147483648.0) || _futureio_f64_lt(af, -2147483648.0))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_s/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_s/i32 unrepresentable");
         return f64_to_i32( to_softfloat64(_futureio_f64_trunc( af )), 0, false );
      }
      uint32_t _futureio_f32_trunc_i32u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_futureio_f32_ge(af, 4294967296.0f) || _futureio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_u/i32 unrepresentable");
         return f32_to_ui32( to_softfloat32(_futureio_f32_trunc( af )), 0, false );
      }
      uint32_t _futureio_f64_trunc_i32u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_futureio_f64_ge(af, 4294967296.0) || _futureio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_u/i32 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_u/i32 unrepresentable");
         return f64_to_ui32( to_softfloat64(_futureio_f64_trunc( af )), 0, false );
      }
      int64_t _futureio_f32_trunc_i64s( float af ) {
         float32_t a = to_softfloat32(af);
         if (_futureio_f32_ge(af, 9223372036854775808.0f) || _futureio_f32_lt(af, -9223372036854775808.0f))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_s/i64 unrepresentable");
         return f32_to_i64( to_softfloat32(_futureio_f32_trunc( af )), 0, false );
      }
      int64_t _futureio_f64_trunc_i64s( double af ) {
         float64_t a = to_softfloat64(af);
         if (_futureio_f64_ge(af, 9223372036854775808.0) || _futureio_f64_lt(af, -9223372036854775808.0))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_s/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_s/i64 unrepresentable");

         return f64_to_i64( to_softfloat64(_futureio_f64_trunc( af )), 0, false );
      }
      uint64_t _futureio_f32_trunc_i64u( float af ) {
         float32_t a = to_softfloat32(af);
         if (_futureio_f32_ge(af, 18446744073709551616.0f) || _futureio_f32_le(af, -1.0f))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f32.convert_u/i64 unrepresentable");
         return f32_to_ui64( to_softfloat32(_futureio_f32_trunc( af )), 0, false );
      }
      uint64_t _futureio_f64_trunc_i64u( double af ) {
         float64_t a = to_softfloat64(af);
         if (_futureio_f64_ge(af, 18446744073709551616.0) || _futureio_f64_le(af, -1.0))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_u/i64 overflow");
         if (is_nan(a))
            FC_THROW_EXCEPTION( futureio::chain::wasm_execution_error, "Error, f64.convert_u/i64 unrepresentable");
         return f64_to_ui64( to_softfloat64(_futureio_f64_trunc( af )), 0, false );
      }
      float _futureio_i32_to_f32( int32_t a )  {
         return from_softfloat32(i32_to_f32( a ));
      }
      float _futureio_i64_to_f32( int64_t a ) {
         return from_softfloat32(i64_to_f32( a ));
      }
      float _futureio_ui32_to_f32( uint32_t a ) {
         return from_softfloat32(ui32_to_f32( a ));
      }
      float _futureio_ui64_to_f32( uint64_t a ) {
         return from_softfloat32(ui64_to_f32( a ));
      }
      double _futureio_i32_to_f64( int32_t a ) {
         return from_softfloat64(i32_to_f64( a ));
      }
      double _futureio_i64_to_f64( int64_t a ) {
         return from_softfloat64(i64_to_f64( a ));
      }
      double _futureio_ui32_to_f64( uint32_t a ) {
         return from_softfloat64(ui32_to_f64( a ));
      }
      double _futureio_ui64_to_f64( uint64_t a ) {
...
class big_int_api : public context_aware_api {
   public:
      ...
      void big_int_pow_mod(array_ptr<char> rop, size_t rop_len,
            ...
            mpz_t n,msg,key,res;
            mpz_init(res);
            FUTURE_ASSERT(0==mpz_init_set_str(n,N.value,base),crypto_api_exception,"Error! Input string can't convert to number");
            FUTURE_ASSERT(0==mpz_init_set_str(msg,Msg.value,base),crypto_api_exception,"Error! Input string can't convert to number");
            FUTURE_ASSERT(0==mpz_init_set_str(key,Key.value,base),crypto_api_exception,"Error! Input string can't convert to number");
            mpz_powm(res,msg,key,n);
            // should check rop_len with res.length
            FUTURE_ASSERT(mpz_get_str(rop.value,base,res)!=NULL,crypto_api_exception,"Error! Result can't convert to string");
      ...
      */
      void big_int_gcd(array_ptr<char> rop, size_t rop_len, null_terminated_ptr p, null_terminated_ptr q,int base) {
            mpz_t P,Q,Res;
            mpz_init(Res);
            FUTURE_ASSERT(0==mpz_init_set_str(P,p.value,base),crypto_api_exception,"Error! Input string can't convert to number");
            FUTURE_ASSERT(0==mpz_init_set_str(Q,q.value,base),crypto_api_exception,"Error! Input string can't convert to number");
            mpz_gcd(Res,P,Q);
            FUTURE_ASSERT(mpz_get_str(rop.value,base,Res)!=NULL,crypto_api_exception,"Error! Result can't convert to string");
      ...
      */
      ...
      void big_int_mul(array_ptr<char> rop, size_t rop_len, null_terminated_ptr p, null_terminated_ptr q,int base){
            mpz_t P,Q,Res;
            mpz_init(Res);
            FUTURE_ASSERT(0==mpz_init_set_str(P,p,base),crypto_api_exception,"Error! Input string can't convert to number");
            FUTURE_ASSERT(0==mpz_init_set_str(Q,q,base),crypto_api_exception,"Error! Input string can't convert to number");
            mpz_mul(Res,P,Q);
            FUTURE_ASSERT(mpz_get_str(rop,base,Res)!=NULL,crypto_api_exception,"Error! Result can't convert to string");
            mpz_clears(Res,P,Q,NULL);
      }
      /**
...
class crypto_api : public context_aware_api {
   public:
      ...
      void frombase58_recover_key(null_terminated_ptr pubkey,
         ...
         FUTURE_ASSERT( hexstr.size() == publen, crypto_api_exception, "frombase58_recover_key public key parase error" );
...
#ifdef FUTURE_TRX_SUPPORT_GM
          //TODO(xiaofen.qin@gmail.com)
          FUTURE_ASSERT(false, crypto_api_exception, "FUTURE_TRX_SUPPORT_GM not provide recover_key");
          return 0;
#else
         signature_type s;
         datastream<const char*> ds( sig, siglen );
      ...
      void assert_sha256(array_ptr<char> data, size_t datalen, const fc::sha256& hash_val) {
         auto result = encode<fc::sha256::encoder>( data, datalen );
         FUTURE_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         FUTURE_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
...
public:
   ...
   :context_aware_api(ctx,true){
      ...
      #ifdef FUTURE_CONFIG_CONTRACT_PARAMS
   ...
   void abort() {
      edump(("abort() called"));
      FUTURE_ASSERT( false, abort_called, "abort() called");
   }
   ...
   void uabort(null_terminated_ptr msg, null_terminated_ptr fileName, int32_t lineNum, int32_t colNum) {
      ...
      edump((message));
      FUTURE_ASSERT( false, abort_called, "abort: " + message);
   }

   // Kept as intrinsic rather than implementing on WASM side (using futureio_assert_message and strlen) because strlen is faster on native side.
   void futureio_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         std::string message( msg );
         edump((message));
         FUTURE_THROW( futureio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }

   void futureio_assert_message( bool condition, array_ptr<const char> msg, size_t msg_len ) {
      if( BOOST_UNLIKELY( !condition ) ) {
   ...
   void futureio_assert_code( bool condition, uint64_t error_code ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         edump((error_code));
         FUTURE_THROW( futureio_assert_code_exception,
                    "assertion failure with error code: ${error_code}", ("error_code", error_code) );
      }
   }

   void futureio_exit(int32_t code) {
...
#define DB_API_METHOD_WRAPPERS_ARRAY_SECONDARY(IDX, ARR_SIZE, ARR_ELEMENT_TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, array_ptr<const ARR_ELEMENT_TYPE> data, size_t data_len) {\
         FUTURE_ASSERT( data_len == ARR_SIZE,\
                    db_api_exception,\
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         FUTURE_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         context.check_rw_db_ability(); \
         return context.IDX.store( scope, table, payer, id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, size_t length) {
         FUTURE_ASSERT((std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, size_t data_len ) {
         ...
         FUTURE_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         FUTURE_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
...
/*
 * This api will be removed with fix for `future #2561`
 */
...
REGISTER_INTRINSICS(typescript_crypto_api,
   ...
   (ts_check_shielded_transaction, int(int, int)                  )
   (ts_update_inc_merkle_tree, int(int, int, int32_t, int, int, int, int, int, int)   )
#endif
);
#endif
...
REGISTER_INTRINSICS(context_free_system_api,
   ...
   (futureio_assert,         void(int, int)      )
   (futureio_assert_message, void(int, int, int) )
   (futureio_assert_code,    void(int, int64_t)  )
   (futureio_exit,           void(int)           )
...
REGISTER_INJECTED_INTRINSICS(softfloat_api,
      (_futureio_f32_add,       float(float, float)    )
      (_futureio_f32_sub,       float(float, float)    )
      (_futureio_f32_mul,       float(float, float)    )
      (_futureio_f32_div,       float(float, float)    )
      (_futureio_f32_min,       float(float, float)    )
      (_futureio_f32_max,       float(float, float)    )
      (_futureio_f32_copysign,  float(float, float)    )
      (_futureio_f32_abs,       float(float)           )
      (_futureio_f32_neg,       float(float)           )
      (_futureio_f32_sqrt,      float(float)           )
      (_futureio_f32_ceil,      float(float)           )
      (_futureio_f32_floor,     float(float)           )
      (_futureio_f32_trunc,     float(float)           )
      (_futureio_f32_nearest,   float(float)           )
      (_futureio_f32_eq,        int(float, float)      )
      (_futureio_f32_ne,        int(float, float)      )
      (_futureio_f32_lt,        int(float, float)      )
      (_futureio_f32_le,        int(float, float)      )
      (_futureio_f32_gt,        int(float, float)      )
      (_futureio_f32_ge,        int(float, float)      )
      (_futureio_f64_add,       double(double, double) )
      (_futureio_f64_sub,       double(double, double) )
      (_futureio_f64_mul,       double(double, double) )
      (_futureio_f64_div,       double(double, double) )
      (_futureio_f64_min,       double(double, double) )
      (_futureio_f64_max,       double(double, double) )
      (_futureio_f64_copysign,  double(double, double) )
      (_futureio_f64_abs,       double(double)         )
      (_futureio_f64_neg,       double(double)         )
      (_futureio_f64_sqrt,      double(double)         )
      (_futureio_f64_ceil,      double(double)         )
      (_futureio_f64_floor,     double(double)         )
      (_futureio_f64_trunc,     double(double)         )
      (_futureio_f64_nearest,   double(double)         )
      (_futureio_f64_eq,        int(double, double)    )
      (_futureio_f64_ne,        int(double, double)    )
      (_futureio_f64_lt,        int(double, double)    )
      (_futureio_f64_le,        int(double, double)    )
      (_futureio_f64_gt,        int(double, double)    )
      (_futureio_f64_ge,        int(double, double)    )
      (_futureio_f32_promote,    double(float)         )
      (_futureio_f64_demote,     float(double)         )
      (_futureio_f32_trunc_i32s, int(float)            )
      (_futureio_f64_trunc_i32s, int(double)           )
      (_futureio_f32_trunc_i32u, int(float)            )
      (_futureio_f64_trunc_i32u, int(double)           )
      (_futureio_f32_trunc_i64s, int64_t(float)        )
      (_futureio_f64_trunc_i64s, int64_t(double)       )
      (_futureio_f32_trunc_i64u, int64_t(float)        )
      (_futureio_f64_trunc_i64u, int64_t(double)       )
      (_futureio_i32_to_f32,     float(int32_t)        )
      (_futureio_i64_to_f32,     float(int64_t)        )
      (_futureio_ui32_to_f32,    float(int32_t)        )
      (_futureio_ui64_to_f32,    float(int64_t)        )
      (_futureio_i32_to_f64,     double(int32_t)       )
      (_futureio_i64_to_f64,     double(int64_t)       )
      (_futureio_ui32_to_f64,    double(int32_t)       )
      (_futureio_ui64_to_f64,    double(int64_t)       )
);
...
std::istream& operator>>(std::istream& in, wasm_interface::vm_type& runtime) {
   ...
   in >> s;
   if (s == "wavm")
      runtime = futureio::chain::wasm_interface::vm_type::wavm;
   else if (s == "binaryen")
      runtime = futureio::chain::wasm_interface::vm_type::binaryen;
   else if (s == "wabt")
      runtime = futureio::chain::wasm_interface::vm_type::wabt;
   else
...
static std::array<std::string, 3> s_zkp_fns = { {
   "ts_verify_zero_knowledge_proof",
   "ts_check_shielded_transaction",
   "ts_update_inc_merkle_tree"
} };
