...
namespace futureio { namespace chain {
   ...
   ,pseudo_start(s)
   {
      ...
      FUTURE_ASSERT( trx.transaction_extensions.size() == 0, unsupported_feature, "we don't support any extensions yet" );
   }

   void transaction_context::init(uint64_t initial_net_usage)
   {
      FUTURE_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
   ...
   void transaction_context::preset_action_ability() {
      auto set_ability_func = [this](action& a) {
         ...
         // futio.rand is a special case where it has pureview action 'query'.
         // otherwise we filter out system account.
         if (account != "futio.rand" &&
             account.size() > 6 &&
             account.substr(0, 6) == "futio.") return;
   ...
   void transaction_context::exec() {
      FUTURE_ASSERT( is_initialized, transaction_exception, "must first initialize" );
   ...
   void transaction_context::check_net_usage()const {
      ...
      if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
         if ( net_limit_due_to_block ) {
            FUTURE_THROW( block_net_usage_exceeded,
            ...
            FUTURE_THROW( greylist_net_usage_exceeded,
                       "net usage of transaction is too high: ${net_usage} > ${net_limit}",
                       ("net_usage", net_usage)("net_limit", eager_net_limit) );
         } else {
            FUTURE_THROW( tx_net_usage_exceeded,
                       "net usage of transaction is too high: ${net_usage} > ${net_limit}",
                       ("net_usage", net_usage)("net_limit", eager_net_limit) );
         }
      }
   ...
   void transaction_context::checktime()const {
      ...
      if( BOOST_UNLIKELY( now > _deadline ) ) {
         ...
         if( explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value ) {
            ...
            FUTURE_THROW( deadline_exception, "deadline exceeded", ("now", now)("deadline", _deadline)("start", start) );
         } else if( deadline_exception_code == block_cpu_usage_exceeded::code_value ) {
            FUTURE_THROW( block_cpu_usage_exceeded,
                       "not enough time left in block to complete executing transaction",
                       ("now", now)("deadline", _deadline)("start", start)("billing_timer", now - pseudo_start) );
         } else if( deadline_exception_code == tx_cpu_usage_exceeded::code_value ) {
            FUTURE_THROW( tx_cpu_usage_exceeded,
                       "transaction was executing for too long",
                       ("now", now)("deadline", _deadline)("start", start)("billing_timer", now - pseudo_start) );
         } else if( deadline_exception_code == leeway_deadline_exception::code_value ) {
            FUTURE_THROW( leeway_deadline_exception,
                       "the transaction was unable to complete by deadline, "
                       "but it is possible it could have succeeded if it were allowed to run to completion",
                       ("now", now)("deadline", _deadline)("start", start)("billing_timer", now - pseudo_start) );
         }
         FUTURE_ASSERT( false,  transaction_exception, "unexpected deadline exception code" );
      }
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, bool check_minimum )const {
      ...
      if( check_minimum ) {
         const auto& cfg = control.get_global_properties().configuration;
         FUTURE_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
      ...
      if( billing_timer_exception_code == block_cpu_usage_exceeded::code_value ) {
         FUTURE_ASSERT( billed_us <= objective_duration_limit.count(),
   ...
   void transaction_context::schedule_transaction() {
       for( const auto& act : trx.actions ) {
           FUTURE_ASSERT(act.account == N(futureio), transaction_exception, "only futureio contract transaction can be delayed" );
           bool has_futureio = false;
           for (const auto& u : act.authorization) {
               if (u.actor == N(futureio)) {
                   has_futureio = true;
                   break;
               }
           }
           FUTURE_ASSERT(has_futureio, transaction_exception, "only futureio can issue delay transaction" );
       }
   ...
   void transaction_context::record_transaction( const transaction_id_type& id, fc::time_point_sec expire ) {
      try {
         ...
         throw;
          ...
          FUTURE_ASSERT( false, tx_duplicate,
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      for( const auto& a : trx.context_free_actions ) {
         ...
         FUTURE_ASSERT( code != nullptr, transaction_exception,
                     "action's code account '${account}' does not exist", ("account", a.account) );
         FUTURE_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            FUTURE_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            FUTURE_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      FUTURE_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
