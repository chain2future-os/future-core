...
#include <futureio/chain/authority_checker.hpp>
...
namespace futureio { namespace chain {
   ...
   namespace detail {
      ...
      struct worldstate_row_traits<permission_object> {
         ...
         static worldstate_permission_object to_worldstate_row(const permission_object& value, const chainbase::database& db, void* data) {
            ...
            FUTURE_ASSERT( itr != idx.end(), worldstate_exception, "Not find the expected parent: ${s}", ("s", value.parent._id));
            ...
            if(value.id != 0){
               ...
               FUTURE_ASSERT( itr_usage != idx_usage.end(), worldstate_exception, "Not find the expected idx_usage: ${s}", ("s", value.usage_id._id));

               const auto& usage = *itr_usage;
               res.last_used = usage.last_used;

               FUTURE_ASSERT( data != nullptr, worldstate_exception, "Data is nullptr, request data!");
         ...
         static void from_worldstate_row(worldstate_permission_object&& row, permission_object& value, chainbase::database& db, bool backup, void* data = nullptr) {
            ...
            if (value.id == 0) {
               FUTURE_ASSERT(row.parent == permission_name(), worldstate_exception, "Unexpected parent name on reserved permission 0");
               FUTURE_ASSERT(row.name == permission_name(), worldstate_exception, "Unexpected permission name on reserved permission 0");
               FUTURE_ASSERT(row.owner == name(), worldstate_exception, "Unexpected owner name on reserved permission 0");
               FUTURE_ASSERT(row.auth.accounts.size() == 0,  worldstate_exception, "Unexpected auth accounts on reserved permission 0");
               FUTURE_ASSERT(row.auth.keys.size() == 0,  worldstate_exception, "Unexpected auth keys on reserved permission 0");
               FUTURE_ASSERT(row.auth.waits.size() == 0,  worldstate_exception, "Unexpected auth waits on reserved permission 0");
               FUTURE_ASSERT(row.auth.threshold == 0,  worldstate_exception, "Unexpected auth threshold on reserved permission 0");
               FUTURE_ASSERT(row.last_updated == time_point(),  worldstate_exception, "Unexpected auth last updated on reserved permission 0");
               value.parent = 0;
               ...
               if(!backup) {
                  ...
                  FUTURE_ASSERT(parent.id != 0, worldstate_exception, "Unexpected mapping to reserved permission 0");
                  ...
                  FUTURE_ASSERT( itr != idx.end(), worldstate_exception, "Not find the expected parent: ${s}", ("s", row.parent));
   ...
   { try {
      FUTURE_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } FUTURE_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
   ...
   } FUTURE_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission account: ${account}", ("account", account) ) }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          FUTURE_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::futureio_any_name )
            return optional<permission_name>();
   ...
   {
      FUTURE_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      FUTURE_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      FUTURE_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      FUTURE_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      FUTURE_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      FUTURE_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link futureio::updateauth to a minimum permission" );
      FUTURE_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link futureio::deleteauth to a minimum permission" );
      FUTURE_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link futureio::linkauth to a minimum permission" );
      FUTURE_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link futureio::unlinkauth to a minimum permission" );
      FUTURE_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link futureio::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to futio.any permission
         return;

      FUTURE_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      FUTURE_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      FUTURE_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::futureio_any_name )
         return;

      FUTURE_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      FUTURE_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      FUTURE_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
      ...
      FUTURE_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is futio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  FUTURE_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         FUTURE_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         FUTURE_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      FUTURE_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            FUTURE_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
