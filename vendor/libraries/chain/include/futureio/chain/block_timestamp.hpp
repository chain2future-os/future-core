   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            FUTURE_ASSERT( std::numeric_limits<uint32_t>::max() - abstime > (config::block_interval_ms / 1000),
...
FC_REFLECT(futureio::chain::block_timestamp_type, (abstime))

namespace fc {
  void to_variant(const futureio::chain::block_timestamp& t, fc::variant& v);
  void from_variant(const fc::variant& v, futureio::chain::block_timestamp& t);
}
