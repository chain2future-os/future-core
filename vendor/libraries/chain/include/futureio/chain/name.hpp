...
#define N(X) futureio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<futureio::chain::name> : private hash<uint64_t> {
      typedef futureio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const futureio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, futureio::chain::name& check);
} // fc


FC_REFLECT( futureio::chain::name, (value) )
