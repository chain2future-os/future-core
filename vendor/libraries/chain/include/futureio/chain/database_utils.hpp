/**
 *  @file
 *  @copyright defined in futureio/LICENSE.txt
 */
#pragma once
...
namespace fc {
   ...
   inline
   void to_variant( const futureio::chain::shared_string& s, variant& v ) {
      v = variant(std::string(s.begin(), s.end()));
   }

   inline
   void from_variant( const variant& v, futureio::chain::shared_string& s ) {
      string _s;
      from_variant(v, _s);
      s = futureio::chain::shared_string(_s.begin(), _s.end(), s.get_allocator());
   }
   ...
   template<typename T>
   void to_variant( const futureio::chain::shared_vector<T>& sv, variant& v ) {
      to_variant(std::vector<T>(sv.begin(), sv.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, futureio::chain::shared_vector<T>& sv ) {
      std::vector<T> _v;
      from_variant(v, _v);
      sv = futureio::chain::shared_vector<T>(_v.begin(), _v.end(), sv.get_allocator());
   }

   template<typename T>
   void to_variant( const futureio::chain::shared_set<T>& s, variant& v ) {
      to_variant(std::vector<T>(s.begin(), s.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, futureio::chain::shared_set<T>& s ) {
      std::vector<T> _v;
      from_variant(v, _v);
      s = futureio::chain::shared_set<T>(_v.begin(), _v.end(), s.get_allocator());
   }
}
...
DataStream& operator << ( DataStream& ds, const float128_t& v ) {
   fc::raw::pack(ds, *reinterpret_cast<const futureio::chain::uint128_t*>(&v));
   return ds;
}

template<typename DataStream>
DataStream& operator >> ( DataStream& ds, float128_t& v ) {
   fc::raw::unpack(ds, *reinterpret_cast<futureio::chain::uint128_t*>(&v));
   return ds;
}
