...
//      }
   ...
   // to its 6-bit slot starting with the highest slot for the first char.
    ...
    #define NEX(X) futureio::chain::string_to_name_ex(#X)
} } // futureio::chain

namespace std {
   template<> struct hash<futureio::chain::name_ex> : private hash<uint64_t> {
      typedef futureio::chain::name_ex argument_type;
...
namespace fc {
  class variant;
  void to_variant(const futureio::chain::name_ex& c, fc::variant& v);
  void from_variant(const fc::variant& v, futureio::chain::name_ex& check);
} // fc


FC_REFLECT( futureio::chain::name_ex, (valueH)(valueL) )
