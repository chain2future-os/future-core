...
#include <futureio/chain/webassembly/wavm.hpp>
...
using namespace fc;
using namespace futureio::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
      wasm_interface_impl(wasm_interface::vm_type vm) {
         if(vm == wasm_interface::vm_type::wavm)
            ...
            FUTURE_THROW(wasm_exception, "wasm_interface_impl fall through");
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            FUTURE_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            FUTURE_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, FUTUREIO_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
