...
const static uint64_t system_account_name    = N(futureio);
const static uint64_t null_account_name      = N(futio.null);

const static uint64_t resource_account_name    = N(futio.res);
const static uint64_t futureio_auth_scope       = N(futio.auth);
const static uint64_t futureio_all_scope        = N(futio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t futureio_any_name = N(futio.any);
const static uint64_t futureio_code_name = N(futio.code);
...
const static futureio::chain::wasm_interface::vm_type default_wasm_runtime = futureio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace futureio::chain::config

constexpr uint64_t FUTURE_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / futureio::chain::config::percent_100;
}

template<typename Number>
Number FUTURE_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + futureio::chain::config::percent_100 - futureio::chain::config::percent_1)  / futureio::chain::config::percent_100;
}
