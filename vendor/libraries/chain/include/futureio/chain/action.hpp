...
namespace futureio { namespace chain {
    ...
    *  In the context of futureio, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         FUTURE_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         FUTURE_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT_ENUM( futureio::chain::action::AbilityType, (Normal)(PureView) )
FC_REFLECT( futureio::chain::permission_level, (actor)(permission) )
FC_REFLECT( futureio::chain::block_reward , (consensus_period)(reward) )
FC_REFLECT( futureio::chain::action, (account)(name)(authorization)(data) )
