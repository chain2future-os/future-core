...
namespace futureio { namespace chain { namespace webassembly { namespace wabt_runtime {
...
struct wabt_apply_instance_vars {
   ...
   char* get_validated_pointer(uint32_t offset, uint32_t size) {
      FUTURE_ASSERT(memory, wasm_execution_error, "access violation");
      FUTURE_ASSERT(offset + size <= memory->data.size() && offset + size >= offset, wasm_execution_error, "access violation");
...
class wabt_runtime : public futureio::chain::wasm_runtime_interface {
   public:
      wabt_runtime();
...
#define _REGISTER_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static futureio::chain::webassembly::wabt_runtime::intrinsic_registrator _INTRINSIC_NAME(__wabt_intrinsic_fn, __COUNTER__) (\
      MOD,\
      NAME,\
      futureio::chain::webassembly::wabt_runtime::wabt_function_type_provider<WASM_SIG>::type(),\
      futureio::chain::webassembly::wabt_runtime::intrinsic_function_invoker_wrapper<SIG>::type::fn<&CLS::METHOD>()\
   );\

} } } }// futureio::chain::webassembly::wabt_runtime
