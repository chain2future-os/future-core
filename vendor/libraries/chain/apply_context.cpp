...
void apply_context::exec()
{
   ...
   if( _cfa_inline_actions.size() > 0 || _inline_actions.size() > 0 ) {
      FUTURE_ASSERT( recurse_depth < control.get_global_properties().configuration.max_inline_action_depth,
...
void apply_context::require_authorization( const account_name& account ) {
   ...
   FUTURE_ASSERT( false, missing_auth_exception, "missing authority of ${account}", ("account",account));
}
...
void apply_context::require_authorization(const account_name& account,
  ...
  FUTURE_ASSERT( false, missing_auth_exception, "missing authority of ${account}/${permission}",
...
void apply_context::update_action_ability(action& a) {
   ...
   FUTURE_ASSERT( code != nullptr, action_validate_exception,
   ...
   if (act.ability == action::PureView) {
      ...
      FUTURE_ASSERT( t != actionDefs.end(), action_validate_exception,
...
void apply_context::execute_inline( action&& a ) {
   ...
   for( const auto& auth : a.authorization ) {
      ...
      FUTURE_ASSERT( actor != nullptr, action_validate_exception,
                  "inline action's authorizing actor ${account} does not exist", ("account", auth.actor) );
      FUTURE_ASSERT( control.get_authorization_manager().find_permission(auth) != nullptr, action_validate_exception,
      ...
      try {
         control.get_authorization_manager()
                .check_authorization( {a},
                                      {},
                                      {{receiver, config::futureio_code_name}},
         ...
         if( disallow_send_to_self_bypass || !send_to_self ) {
            ...
            FUTURE_THROW(subjective_block_production_exception, "Unexpected exception occurred validating inline action sent to self");
         }
      }
   }
...
void apply_context::execute_context_free_inline( action&& a ) {
   update_action_ability(a);
   FUTURE_ASSERT( a.authorization.size() == 0, action_validate_exception,
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
   FUTURE_ASSERT(act.account == config::system_account_name || act.account == N(futio.msig) || act.account == config::resource_account_name, action_validate_exception, "only futureio,futio.msig,futio.res contract can send deferred transaction" );
   FUTURE_ASSERT( trx.context_free_actions.size() == 0, cfa_inside_generated_tx, "context free actions are not currently allowed in generated transactions" );
   ...
   if( !control.skip_auth_check() && !privileged ) { // Do not need to check authorization if replayng irreversible block or if contract is privileged
      ...
      try {
         ...
         if( disallow_send_to_self_bypass || !is_sending_only_to_self(receiver) ) {
            ...
            FUTURE_THROW(subjective_block_production_exception, "Unexpected exception occurred validating sent deferred transaction consisting only of actions to self");
         }
      }
   }
   ...
   if ( auto ptr = d.find<generated_transaction_object,by_sender_id>(boost::make_tuple(receiver, sender_id)) ) {
      FUTURE_ASSERT( replace_existing, deferred_tx_duplicate, "deferred transaction with the same sender_id and payer already exists" );

      // TODO: Remove the following subjective check when the deferred trx replacement RAM bug has been fixed with a hard fork.
      FUTURE_ASSERT( !control.is_producing_block(), subjective_block_production_exception,
...
void apply_context::update_db_usage( const account_name& payer, int64_t delta ) {
   auto p = receiver;
   //if(delta < 0 && (receiver == N(futureio) || receiver == config::resource_account_name)){
   if(delta < 0 && receiver == N(futureio) ) {
      p = payer;
   }
//    if( delta > 0 ) {
...
int apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   FUTURE_ASSERT(act_ptr, action_not_found_exception, "action is not found" );
...
void apply_context::check_rw_db_ability() const {
   ...
   FUTURE_ASSERT( act.ability == action::Normal, table_access_violation, "pureview action can not store, modify or erase item(s) from table.");
}
...
//   require_write_lock( scope );
   ...
   FUTURE_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
...
void apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   ...
   FUTURE_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
int apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   ...
   if( iterator < -1 ) // is end iterator
   {
      ...
      FUTURE_ASSERT( tab, invalid_table_iterator, "not a valid end iterator" );
...
int apply_context::db_drop_secondary_index(const futureio::chain::table_id_object * t_id, uint64_t& calc_num, uint64_t& calc_delram_bytes) {
   ...
   FUTURE_ASSERT( systemname == receiver || resourcename == receiver, table_access_violation, "db access violation" );
