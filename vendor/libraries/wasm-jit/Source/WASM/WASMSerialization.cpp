#include "../../../chain/include/futureio/chain/wasm_futureio_constraints.hpp"
...
namespace WASM
{
	...
	void serializeFunctionBody(InputStream& sectionStream,Module& module,FunctionDef& functionDef)
	{
		...
		constexpr size_t max_size = futureio::chain::wasm_constraints::maximum_code_size;
		...
		for(Uptr setIndex = 0;setIndex < numLocalSets;++setIndex)
		{
			...
			if( locals_accum > futureio::chain::wasm_constraints::maximum_func_local_bytes )
	...
	void serializeImportSection(Stream& moduleStream,Module& module)
	{
		serializeSection(moduleStream,SectionType::import,[&module](Stream& sectionStream)
		{
			...
			constexpr size_t max_size = futureio::chain::wasm_constraints::maximum_section_elements;
			if(Stream::isInput)
			{
