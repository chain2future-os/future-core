...
typedef std::vector<unsigned char> DynamicEncCiphertext;
typedef std::vector<unsigned char> DynamicEncPlaintext;
...
typedef std::vector<unsigned char> DynamicOutCiphertext;
typedef std::vector<unsigned char> DynamicOutPlaintext;
...
public:
    ...
    boost::optional<DynamicEncCiphertext> encrypt_to_recipient(
        const zero::uint256 &pk_d,
        const DynamicEncPlaintext &message
    );
    ...
    boost::optional<DynamicOutCiphertext> encrypt_to_ourselves(
        const zero::uint256 &pk_d,
        const DynamicOutPlaintext &message
    );
...
// Attempts to decrypt a Sapling note. This will not check that the contents
// of the ciphertext are correct.
boost::optional<DynamicEncPlaintext> AttemptSaplingEncDecryption(
...
// Attempts to decrypt a Sapling note using outgoing plaintext.
// This will not check that the contents of the ciphertext are correct.
boost::optional<DynamicOutPlaintext> AttemptSaplingOutDecryption(
    const DynamicOutCiphertext &ciphertext,
    const zero::uint256 &ivk,
    const zero::uint256 &epk
);
