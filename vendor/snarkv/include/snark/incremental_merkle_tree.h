...
public:
    ...
    void toBytes(std::vector<unsigned char>& witness_path) {
        MerklePathPack pack;
        toPack(pack);
        size_t size = 1;
        for (auto& pv : pack.path_bytes) {
            size += 1;
            size += pv.size();
        }
        size += sizeof(uint64_t);

        witness_path.resize(size);
        int i = 0;
        witness_path[i++] = pack.path_bytes.size();

        for (auto& pv : pack.path_bytes) {
            witness_path[i++] = pv.size();
            for (auto byte : pv) {
                witness_path[i++] = byte;
            }
        }
        memcpy(&(witness_path[i]), &pack.index_int, sizeof(uint64_t));
    }
};
