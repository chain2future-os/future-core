...
namespace futureiosystem {
    struct committee_info {
        ...
        FUTURELIB_SERIALIZE(committee_info, (owner)(producer_key)(bls_key) )
        ...
        bool is_empty() const {
            std::string accountstr = futureio::name{owner}.to_string();
        ...
        void to_strstream(std::string& s) const {
            ...
            std::string accountstr = futureio::name{owner}.to_string(); //to remove
        ...
        bool from_strstream(const std::string& s, size_t start, size_t& next) {
            ...
            owner = futureio::string_to_name(acc_name.c_str());
