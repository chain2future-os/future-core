...
#include <futureiolib/public_key.hpp>
...
namespace futureiosystem {
   struct permission_level_weight {
      futureio::permission_level  permission;
      weight_type       weight;

      FUTURELIB_SERIALIZE( permission_level_weight, (permission)(weight) )
   };

   struct key_weight {
      futureio::public_key  key;
      weight_type        weight;

      FUTURELIB_SERIALIZE( key_weight, (key)(weight) )
   };
   ...
   struct wait_weight {
      ...
      FUTURELIB_SERIALIZE( wait_weight, (wait_sec)(weight) )
   };
   ...
   struct authority {
      ...
      FUTURELIB_SERIALIZE( authority, (threshold)(keys)(accounts)(waits) )
   };
   ...
   struct action_actor_type {
      ...
      FUTURELIB_SERIALIZE( action_actor_type, (actor)(action) )
   };

   class native : public futureio::contract {
      public:

         using futureio::contract::contract;
         ...
         void onerror( /*const bytes&*/ ) { require_auth( N(futureio) );}
         ...
         void addwhiteblack( /*std::vector<account_name>           actor_black;
                           ...
                           std::vector<futureio::public_key>  key_black; */) { }
