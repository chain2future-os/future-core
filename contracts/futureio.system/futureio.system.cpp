...
namespace futureiosystem {
   ...
   futureio_global_state system_contract::get_default_parameters() {
      futureio_global_state dp;
   ...
   void system_contract::setsysparams( const futureio_system_params& params) {
        require_auth( _self );
        if (params.chain_type == 0) { //master chain
            _gstate.max_ram_size = params.max_ram_size;
            futureio_assert( params.max_ram_size > _gstate.total_ram_bytes_used,
            ...
            if ( _gstate.cur_committee_number > _gstate.min_committee_member_number ) { //genesis bios already ended
                futureio_assert( _gstate.cur_committee_number > params.min_committee_member_number,
            ...
            futureio_assert(ite_chain != _chains.end(), "this chian does not exist");
   ...
   void system_contract::setglobalextendata( uint16_t key, std::string value ) {
      require_auth( _self );
      futureio_assert( !value.empty(), " global value is empty" );
      futureio_assert( key < futureio_global_state::global_state_exten_type_key::global_state_key_end, " key should not exist" );
   ...
   void system_contract::setchaintypeextendata( uint64_t type_id, uint16_t key, std::string value ) {
      require_auth( _self );
      futureio_assert( !value.empty(), "setchaintypeextendata value is empty" );
      futureio_assert( key < chaintype::chaintype_exten_key::chaintype_key_end, " key should not exist" );
      bool is_exist_key = false;
      chaintypes_table type_tbl( _self, _self );
      auto type_iter = type_tbl.find( type_id );
      futureio_assert( type_iter != type_tbl.end(), "setchaintypeextendata type_id not found" );
   ...
   uint64_t system_contract::getchaintypeextenuintdata( const futureiosystem::chaintypes_table::const_iterator& type_iter, uint16_t key, uint64_t default_value ) const {
   ...
   void system_contract::setmasterchaininfo( const master_chain_info& chaininfo ){
      ...
      futureio_assert(it_masterchain == masterinfos.end(), "masterinfos already exist");
      ...
      futureio_assert(it == _chains.end(), "master chain already exist");
   ...
   void system_contract::setparams( const futureio::blockchain_parameters& params ) {
      require_auth( N(futureio) );
      (futureio::blockchain_parameters&)(_gstate) = params;
      futureio_assert( 3 <= _gstate.max_authority_depth, "max_authority_depth should be at least 3" );
   ...
   void system_contract::setupdateabled( account_name account, uint8_t is_update ) {
      if( !has_auth(_self) ){
         ...
         futureio_assert( !is_update, "the contract account can only be set to unupdatable" );
         INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {account,N(active)},
            { account, N(futio.fee), asset(1000), std::string("setupdateabled") } );  //0.1 FGAS service charge
      }
   ...
   void system_contract::setprodontimeblock( account_name account, uint32_t online_second, uint32_t blocknum ) {
      ...
      futureio_assert(briefprod != _briefproducers.end(), "this account is not a producer");
      ...
      futureio_assert(!briefprod->in_disable, "producer is disabled");
      ...
      _producers.modify(it, [&](auto & v) {
         ...
         if(online_second != 0){
            futureio_assert(now()>(uint32_t)online_second, "set online second is wrong");
   ...
   void system_contract::update_commercial_delegatesd( name chain_name, asset delegated_value ) {
      ...
      if( ite_com_table == chaincommdeltab.end() ) {
        ...
        chaincommdeltab.modify( ite_com_table, [&]( chain_commercial_delegate& d ) {
            ...
            //futureio_assert( d.delegate_value >= 0, " total_delegated_value shouldn't be negative" );
        });
      }
   }
   ...
   void system_contract::add_subchain_account(const futureio::proposeaccount_info&  newacc ) {
      futureiosystem::key_weight ownerkeyweight;
      ...
      futureiosystem::authority    ownerkey  = { .threshold = 1, .keys = { ownerkeyweight }, .accounts = {}, .waits = {} };

      futureiosystem::key_weight activekeyweight;
      ...
      futureiosystem::authority     activekey = { .threshold = 1, .keys = { activekeyweight }, .accounts = {}, .waits = {} };
      print("updateactiveaccounts proposerminer:",name{newacc.account}," ownerkey:",newacc.owner_key," active_key:",newacc.active_key, "\n");
      action(
         permission_level{ N(futureio), N(active) },
         N(futureio), NEX(newaccount),
         std::make_tuple(N(futureio),newacc.account, ownerkey, activekey, newacc.updateable)
      ).send();
   }

    bool system_contract::accept_block_header(name chain_name, const futureio::signed_block_header& header, char* confirmed_bh_hash, size_t hash_len) {
    ...
    *  apply_futureio_newaccount() in futureio_contract.cpp.
    *
    */
   void native::newaccount( account_name     creator,
      ...
      if( !is_privileged( creator ) ) {
         futureio_assert( name_str.size() >= 5 && name_str.size() <= 12 , "account names must be between 5-12 in length" );
         ...
         futureio_assert( is_contain_letter && is_contain_number , "account names must contain lowercase letters and Numbers (1-5)" );
         futureio_assert( name_str.find( "futio." ) != 0, "only privileged accounts can have names that start with 'futio.'" );
      }
      int32_t newaccount_fee = 0;
      if(globalparams.exists()){
         futureio_global_state  _gstate = globalparams.get();
         newaccount_fee = (int32_t)_gstate.newaccount_fee;
         futureio_assert( _gstate.is_master_chain() || creator == _self, "only master chain allow create account" );
      }
      ...
      if(!is_free_create && creator != _self){
        INLINE_ACTION_SENDER(futureiores::resource, modifyfreeaccount)
                            (N(futio.res), {N(futio.res), N(active)},{creator, 1});
      }
      // if (!is_free_create && creator != _self && newaccount_fee > 0) {
      //    INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {creator,N(active)},
      //                         { creator, N(futio.fee), asset(newaccount_fee), std::string("create account") } );
      // }

      if ( name_str.find( "futio." ) != 0 ) {
   ...
   void native::updateauth( account_name     account,
         ...
         futureio_assert(authsize > 0, "update authority amount has to be greater than zero");

         global_state_singleton globalparams( _self,_self);
         futureio_assert(globalparams.exists(), "global state is not existed");
         futureio_global_state  _gstate = globalparams.get();
         ...
         for(auto extension : _gstate.table_extension){
             if(extension.key == futureio_global_state::global_state_exten_type_key::update_auth) {
         ...
         if(updateauth_fee > 0 && (name{account}.to_string().find( "futio." ) != 0 ) && (account != _self)){
            INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {account,N(active)},
               { account, N(futio.fee), asset(updateauth_fee * authsize), std::string("update auth") } );
         }
      }
   ...
   void native::linkauth( account_name    account,
      ...
      if(globalparams.exists()){
         futureio_global_state  _gstate = globalparams.get();
         for(auto extension : _gstate.table_extension){
            if(extension.key == futureio_global_state::global_state_exten_type_key::link_auth_fee
      ...
      INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), { account,N(active) },
         { account, N(futio.fee), asset( linkauth_fee ), name{account}.to_string() + std::string(" linkauth fee") } );
   }
   ...
   void native::delaccount( account_name account ) {
      require_auth( _self );
      asset account_tokens = futureio::token(N(futio.token)).get_balance( account ,symbol_type(CORE_SYMBOL).name());
      futureio_assert( account_tokens.amount == 0, " The account still has a balance and cannot be deleted" );
      producer_brief_table   _briefproducers( _self, _self );
      auto const briefprod = _briefproducers.find( account );
      futureio_assert(briefprod == _briefproducers.end(), "this account is a producer and cannot be deleted");
      /*
      chains_table _chains(_self,_self);
      for(auto ite_chain = _chains.begin(); ite_chain != _chains.end(); ++ite_chain) {
         if( ite_chain->chain_name == N(master) )
         ...
         futureio_assert( reslease_itr ==  _reslease_tbl.end(), "this account has purchased resource and cannot be deleted");
      }*/
...
FUTUREIO_ABI( futureiosystem::system_contract,
     // native.hpp (newaccount definition is actually in futureio.system.cpp)
     ...
     // producer.cpp
     (regproducer)(moveprod)(verifyprodevil)(procevilprod)(prodheartbeat)(opefixedprod)
     // reward.cpp
