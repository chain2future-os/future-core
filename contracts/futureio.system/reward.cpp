...
#include <futureio.res/futureio.res.hpp>
...
namespace futureiosystem {
   ...
   void system_contract::onblock( block_timestamp timestamp, account_name producer ) {
      ...
      INLINE_ACTION_SENDER(futureiores::resource, onblock)( N(futio.res), { _self, N(active) }, std::make_tuple() );
   }
   float system_contract::get_reward_fee_ratio() const {
      ...
      for( auto extension : _gstate.table_extension ){
         if(extension.key == futureio_global_state::global_state_exten_type_key::sidechain_charge_ratio) {
   ...
   uint64_t system_contract::get_reward_per_block() const {
      ...
      auto const reward_iter = std::find_if( _gstate.block_reward_vec.begin(), _gstate.block_reward_vec.end(), [&](futureio::block_reward const& obj){
   ...
   uint32_t system_contract::check_previous_claimed_reward( const futureiosystem::producer_info& prod, uint32_t block_height ) const {
      ...
      for( auto& exten : prod.table_extension ){
         if( exten.key == producer_info::producers_state_exten_type_key::claim_rewards_block_height ){
            ...
            futureio_assert( (block_height > pre_rewards_blockheight)
   ...
   void system_contract::send_rewards_for_producer( account_name producer, account_name reward_account, const name& chain_name, uint64_t unpaid_balance ) {
      ...
      account_name pay_account = ( chain_name == self_chain_name ) ? N(futio.mfee) : N(futio.resfee) ;
      asset pay_tokens = futureio::token(N(futio.token)).get_balance( pay_account,symbol_type(CORE_SYMBOL).name());
      futureio_assert( pay_tokens.amount >= (int64_t)unpaid_balance, " Insufficient funds to claim rewards" );
      if( unpaid_balance > 10000*10000 ) {
         reward_account = N(futio.reward);
      }
      INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {pay_account,N(active)},
      ...
      futureio_assert( _gstate.total_unpaid_balance >= unpaid_balance, " unpaid balance exception" );
   ...
   //Call this function to ensure that the producer is in the disabled_producers_table
   void system_contract::add_rewards_for_disproducer( account_name producer, account_name reward_account, uint64_t unpaid_balance ) {
      ...
      if ( unprod_itr == _unpaid_disproducer.end() ){
         ...
         _unpaid_disproducer.modify( unprod_itr, [&]( auto& u ) {
            u.unpaid_balance += unpaid_balance;
         });
      }
   }
   ...
   void system_contract::send_rewards_for_maintainer( account_name maintainer ) {
      unpaid_disprod _unpaid_maintainer( _self, N(maintainer) );
      auto unmaint_itr = _unpaid_maintainer.find( maintainer );
      futureio_assert( unmaint_itr != _unpaid_maintainer.end(), " _unpaid_maintainer not exist " );
      futureio_assert( unmaint_itr->unpaid_balance > 0, " _unpaid_maintainer balance is zero " );
   ...
   void system_contract::report_subchain_block( const name& chain_name, account_name producer, uint64_t block_height,
      ...
      auto prod = _producers.find(producer);
      if ( prod == _producers.end() ) {
         print("block proposer ", name{producer}, " is not found in its location\n");
      }
      ...
      if ( prod_itr == _prodhistory.end() ) {
         prod_itr = _prodhistory.emplace([&](producer_history &prodhist){
            ...
            } else if ( it_disable != dp_tbl.end() ) {
               prodhist.total_produce_block = it_disable->total_produce_block;
            }
         });
      }
      ...
      if ( !is_fixed_reward_rate ) {
         ...
         record_rewards_for_maintainer( futureio_community_name, reward_percentage * 5 );
         record_rewards_for_maintainer( futureio_technical_team_name, reward_percentage * 5 );
         record_rewards_for_maintainer( futureio_dapp_name, reward_percentage * 10 );
         ...
         if( prod != _producers.end() ) {
            ...
            add_rewards_for_disproducer( producer, it_disable->claim_rewards_account, realreward );
            dp_tbl.modify(it_disable, [&]( disabled_producer& _dis ) {
               _dis.total_produce_block++;
            });
         }
      } else {
         // For daily fixed rewards features, need to consider producer heartbeat clock and offline disproducer reward issues
         #if 0
   ...
   void system_contract::claimrewards( account_name producer ) {
      futureio_assert( _gstate.is_master_chain(), "It's not that the master chain can't get rewards" );
      ...
      if( producer == futureio_community_name
         || producer == futureio_technical_team_name
         || producer == futureio_dapp_name ){
      ...
      if( briefprod.in_disable ) {  //disabled producer claimrewards
         ...
         futureio_assert( unprod.unpaid_balance != 0, " unpaid balance is zero" );
         ...
         futureio_assert( prod.unpaid_balance != 0, " unpaid balance is zero" );
   ...
   void system_contract::calcmasterrewards() {
      ...
      asset fee_tokens = futureio::token(N(futio.token)).get_balance( N(futio.fee),symbol_type(CORE_SYMBOL).name());
      ...
      if(_gstate.master_chain_pay_fee > 0){
         ...
         asset resfee_tokens = futureio::token(N(futio.token)).get_balance( N(futio.resfee),symbol_type(CORE_SYMBOL).name());
         if( resfee_tokens.amount > _gstate.master_chain_pay_fee )
            INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), { N(futio.resfee),N(active) },
               { N(futio.resfee), N(futio.mfee), asset(_gstate.master_chain_pay_fee), std::string("master producers block pay") } );
         _gstate.master_chain_pay_fee = 0;
      }
      if( master_reward_tokens.amount <= 0 )
         return;
      if ( fee_tokens.amount > 0 ) {
         INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), { N(futio.fee),N(active) },
            { N(futio.fee), N(futio.mfee), fee_tokens, std::string("master producers block pay") } );
      }
   ...
   void system_contract::distribut_reward(uint64_t current_block_height){
      ...
      //distribute rewards
      futureio::transaction rewardtrans;
      rewardtrans.actions.emplace_back(
          permission_level{ _self, N(active) }, _self, NEX(calcmasterrewards), std::make_tuple() );
      rewardtrans.actions[0].authorization.emplace_back(permission_level{ N(futureio), N(active)});
