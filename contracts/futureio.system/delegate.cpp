...
namespace futureiosystem {
   ...
   struct delegated_consensus {
      ...
      FUTURELIB_SERIALIZE( delegated_consensus, (from)(to)(cons_weight) )
   };
    ...
    struct refund_cons {
      ...
      FUTURELIB_SERIALIZE( refund_cons, (owner)(request_time)(cons_amount) )
   };

   /**
    *  user staked consensus
    */
   typedef futureio::multi_index< N(delcons), delegated_consensus> del_consensus_table;
   typedef futureio::multi_index< N(refundscons), refund_cons>     refunds_cons_table;
   ...
   void system_contract::change_cons(account_name from, account_name receiver, asset stake_cons_delta) {
       ...
       futureio_assert(_gstate.is_master_chain() ||  // on master chain
                        from == _self || // futureio account
                        name{from}.to_string().find( "futio." ) == 0, // system account
       ...
       futureio_assert(stake_cons_delta.amount >= 100 * precision ||
                        stake_cons_delta.amount < 0 , "should stake at least 100 FGAS" );
      ...
      if (stake_cons_delta.amount < 0) {
          futureio_assert(itr != del_tbl.end(), "no delegate record found for undelgating action" );
      }
      ...
      futureio_assert( asset(0) <= itr->cons_weight, "insufficient staked consensous" );
      if ( itr->cons_weight == asset(0)) {
         del_tbl.erase( itr );
      }

      account_name source_stake_from = from;
      if ( N(futio.stake) != source_stake_from ) { //for futureio both transfer and refund make no sense
          if (stake_cons_delta.amount < 0) {
              process_undelegate_request(from, stake_cons_delta);
          } else {
            INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {source_stake_from, N(active)},
               { source_stake_from, N(futio.stake), asset(stake_cons_delta), std::string("stake consensus") } );
         }
      }
      ...
      // if on master chain, update voting power; else add to subchain.
      {
         ...
         auto const check_isallow_undelegate_func = [=]( uint64_t delegated_cons_blocknum ){
            ...
            if((name{from}.to_string().find( "futio." ) == 0 ) || (from == _self))
               return;
            uint64_t undelegate_blocks_interval  = getglobalextenuintdata( futureio_global_state::global_state_exten_type_key::allow_undelegate_block_interval,
            ...
            futureio_assert( (curblocknum - delegated_cons_blocknum) > undelegate_blocks_interval , assert_msg.c_str() );
         };
         asset total_update = stake_cons_delta;
         auto briefprod = _briefproducers.find(receiver);
         futureio_assert(briefprod != _briefproducers.end(), "this account is not a producer, please regproducer first");
         ...
         if(briefprod->in_disable) {
            ...
            futureio_assert(dis_prod != dp_tbl.end(), "receiver is not found in its location");
            check_isallow_undelegate_func( dis_prod->delegated_cons_blocknum );
             ...
             if(enabled) {
                prodhistory_table _prodhistory( _self, receiver );
                auto prodhis_itr = _prodhistory.find( receiver );
                if ( prodhis_itr == _prodhistory.end() ) {
                    _prodhistory.emplace([&]( producer_history &prodhist ) {
                        prodhist.pay_per_day.assign(32, {0,0});
                        prodhist.owner = receiver;
                        prodhist.delegated_enable_timestamp = now();
                        prodhist.total_produce_block = dis_prod->total_produce_block;
                    });
                }
             ...
             futureio_assert(it != _producers.end(), "receiver is not found in its location");
    ...
    void system_contract::process_undelegate_request(account_name from,
        ...
        futureio_assert( unstake_quantity < asset(0), "unstaked consensous asset must be negative" );
        ...
        if ( req != refunds_tbl.end() ) { //need to update refund_cons
             ...
             futureio_assert( asset(0) <= req->cons_amount, "negative refund_cons amount" ); //should never happen
        ...
        if ( need_deferred_trx ) {
            ...
            out.delay_sec = (uint32_t)getglobalextenuintdata( futureio_global_state::global_state_exten_type_key::refund_delegate_consensus_seconds,
                                                   refund_delay );
...
/*
   void system_contract::resourcelease( account_name from, account_name receiver,
      ...
      for(auto extension : _gstate.table_extension){
         if(extension.key == futureio_global_state::global_state_exten_type_key::is_allow_buy_res
            && extension.value == "1"){
      ...
      if( !is_allow_buy_res )
         futureio_assert( from == _self, "only allow futureio account resourcelease" );
      futureio_assert(location != default_chain_name && location != N(master) , "wrong location");
      auto chain_itr = _chains.end();
      if(location != self_chain_name) {
         chain_itr = _chains.find(location);
         futureio_assert(chain_itr != _chains.end(), "this subchian location is not existed");
         futureio_assert(is_empowered(receiver, location), "the receiver is not yet empowered to this chain before");
      }else{
          futureio_assert( from == _self, "master chain not allow buy resourcelease" );
      }
      ...
      futureio_assert( combosize <= uint64_t(max_availableused_size), availableuserstr.c_str() );
      uint64_t bytes = 0;
      if(chain_itr == _chains.end()) {
      ...
      futureio_assert( days <= (365*30+7), "resource lease buy days must reserve a positive and less than 30 years" );
      uint32_t  free_account_per_res = 50; //The default free_account_per_res is 50
      for(auto extension : _gstate.table_extension){
         if(extension.key == futureio_global_state::global_state_exten_type_key::free_account_per_res) {
      ...
      // update totals of "receiver"
      {
         ...
         if( reslease_itr ==  _reslease_tbl.end() ) {
            ...
            futureio_assert(deltab_itr == pendingdel.end(), "legacy resource is pending for clean up, please apply new resources later");

            futureio_assert( (combosize > 0) && (days > 0), "resource lease buy days and numbler must > 0" );
            ...
            futureio_assert(reslease_itr->end_block_height > cur_block_height, "legacy resource is waiting for clean up, please apply new resource later");
            uint64_t free_account_number = 0;
            futureio_assert(((combosize > 0) && (days == 0))||((combosize == 0) && (days > 0)), "resource lease days and numbler can't increase them at the same time" );
            uint32_t  blocknumberhour = seconds_per_day/24/block_interval_seconds();
            futureio_assert( (reslease_itr->modify_block_height + blocknumberhour) <  cur_block_height, "Renewal resources cannot be operated within one hour" );
            if(combosize > 0)
            {
               futureio_assert(reslease_itr->end_block_height > cur_block_height, "resource lease endtime already expired" );
            ...
            {
               futureio_assert(reslease_itr->lease_num > 0, "resource lease number is not normal" );
         ...
         if( _gstate.is_master_chain() || from != _self )
            INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {from,N(active)},
                                { from, N(futio.resfee), asset(resourcefee), std::string("buy resource lease") } );
         print("resourcelease calculatefee receiver:", name{receiver}," resourcenum_time:",cuttingfee, " resourcefee:",resourcefee);
         futureio_assert( 0 < reslease_itr->lease_num, "insufficient resource lease" );
         if (chain_itr == _chains.end()) {
   ...
   void system_contract::transferresource(account_name from, account_name to, uint64_t combosize, name location) {
       ...
       if(_gstate.is_master_chain()) {
           require_auth( from );
           futureio_assert( location != self_chain_name, "master chain is not allowed to transfer resourcelease" );

           auto chain_ite = _chains.find(location);
           futureio_assert(chain_ite != _chains.end(), "this subchian location is not existed");
           futureio_assert(is_empowered(to, location), "the receiver was not yet empowered to this chain before");
           bytes_per_combo = chain_ite->global_resource.max_ram_size/chain_ite->global_resource.max_resources_number;
       } else {
           require_auth( _self );
           futureio_assert( location == self_chain_name, "wrong location to tranfer resources at sidechain" );
       ...
       futureio_assert(lease_from != _reslease_tbl.end(), assert_desc.c_str());
       assert_desc = name{from}.to_string();
       assert_desc.append(" has not enought resources to transfer");
       futureio_assert(lease_from->lease_num >= combosize, assert_desc.c_str());
       uint32_t cur_block_height = (uint32_t)head_block_number() + 1;
       futureio_assert(lease_from->end_block_height > cur_block_height, "resource is expired and waiting for clean up");
       ...
       if(lease_to != _reslease_tbl.end()) {
           ...
           futureio_assert(lease_to->end_block_height == lease_from->end_block_height, assert_desc.c_str());
       ...
       if(_gstate.is_master_chain()) {
           ...
           futureio_assert(lease_from->free_account_number >= free_account_size, "no enough free account number");
       } else {
           ...
           futureio_assert(need_bytes <= uint64_t(total_ram - used_ram), assert_desc.c_str());
       ...
       if( _gstate.is_master_chain()) {
           ...
           futureio_assert(days > 0, "resource will expire in one day");
           int64_t fee = int64_t(getglobalextenuintdata(futureio_global_state::res_transfer_fee, asset().symbol.precision()));
           INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {from,N(active)},
                    { from, N(futio.resfee), asset(fee), std::string("transfer resource") } );
       }
...
*/
   ...
   {
      futureio_assert( stake_cons_quantity > asset(0), "must stake a positive amount" );
      change_cons( from, receiver, stake_cons_quantity);
   } // delegatecons

    void system_contract::undelegatecons( account_name from, account_name receiver) {
        auto briefprod = _briefproducers.find(receiver);
        futureio_assert(briefprod != _briefproducers.end(), "Unable to undelegate cons for non-producer" );
        if (!briefprod->in_disable) {
            producers_table _producers(_self, briefprod->location);
            auto prod = _producers.find(receiver);
            futureio_assert( (prod != _producers.end()), "Producer is not found in its location" );
            futureio_assert(_gstate.cur_committee_number > _gstate.min_committee_member_number,
   ...
   void system_contract::refundcons(const account_name owner ) {
      ...
      futureio_assert( req != refunds_tbl.end(), "refunds_cons_table request not found" );
      auto refund_delay_sec = (uint32_t)getglobalextenuintdata( futureio_global_state::global_state_exten_type_key::refund_delegate_consensus_seconds,
                                             refund_delay );
      futureio_assert( req->request_time + refund_delay_sec <= now(), "refund is not available yet" );
      ...
      INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {N(futio.stake),N(active)},
                                                    { N(futio.stake), req->owner, req->cons_amount, std::string("unstake") } );
   ...
   void system_contract::check_res_expire(uint64_t current_block_height){
      ...
      if(1 == getglobalextenuintdata(futureio_global_state::pending_resource_check, 0)) {
      ...
      setglobalextendata(futureio_global_state::pending_resource_check, pending_res_status);
