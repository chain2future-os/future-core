...
namespace futureiosystem {
    ...
    void system_contract::regchaintype(uint64_t type_id, uint16_t min_producer_num, uint16_t max_producer_num,
        ...
        futureio_assert(_gstate.is_master_chain(), "only master chain can register chain type");
        futureio_assert(type_id != 0, "type id 0 has been occupied by future system");
        futureio_assert(min_producer_num >= 4, "wrong min_producer_num, at least 4 producers is required for a chain");
        futureio_assert(min_producer_num < max_producer_num, "max_producer_num must grater than min_producer_num");
        futureio_assert(sched_step > 1 && sched_step <= 100, "sched_step should in scope [2, 100]");
        futureio_assert(consensus_period > 1 && consensus_period <= 10, "consensus_period should in scope [2, 10]");
        futureio_assert(min_activated_stake >= 0, "min_activated_stake should be greater than 0");
    ...
    void system_contract::reportblockwshash(uint64_t blocknum, checksum256 hash) {
        reportwshash(name{N(futio.wshash)}, blocknum, hash, 0, ws_hash_type::ws_hash_validate);
    }
    ...
    void system_contract::reportwshash(name subchain, uint64_t blocknum, checksum256 hash, uint64_t file_size, ws_hash_type hash_type) {
        ...
        if (hash_type == ws_hash_type::ws_hash_validate) {
            check_chain = name{N(futureio)};
        }
        producers_table _producers(_self, check_chain);
        auto propos = _producers.find(sender);
        futureio_assert( propos != _producers.end(), "enabled producer not found this proposer" );
        ...
        if (check_chain == self_chain_name) {
            ...
            futureio_assert(ite_chain != _chains.end(), "subchain not found");
        ...
        futureio_assert(checkBlockNum(blocknum), "report an invalid blocknum ws");
        futureio_assert(blocknum <= confirmed_blocknum, "report a blocknum larger than current block");
        futureio_assert(confirmed_blocknum - blocknum <= _gstate.worldstate_interval, "report a too old blocknum");
        ...
        if(wshash != hashTable.end()) {
            ...
            futureio_assert(ret == acc.end(), "the committee_members already report such ws hash");

            auto it = hashv.begin();
            for(; it != hashv.end(); it++) {
                if(hash == it->hash && file_size == it->file_size) {
                    futureio_assert(it->valid != true, "the hash value has been voted as valid");
                    if((it->votes+1) >= vote_threshold) {
                        ...
                        if(cnt == 0) {
                            ...
                            if (hash_type == ws_hash_type::ws_hash_validate) {
                                ...
                                futureio_assert(blockws == ws_table.end(), "wshash for this block has been recorded");
    ...
    void system_contract::regsubchain(name chain_name, uint64_t chain_type, const std::string& genesis_producer_pubkey) {
        require_auth(N(futureio));
        futureio_assert(_gstate.is_master_chain(), "only master chain can register new chain");
        futureio_assert(chain_name != default_chain_name && chain_name != self_chain_name && chain_name != N(master), "unusable chian name");
        auto itor = _chains.find(chain_name);
        futureio_assert(itor == _chains.end(), "there has been a subchian with this name");
        chaintypes_table type_tbl(_self, _self);
        auto type_iter = type_tbl.find(chain_type);
        futureio_assert(type_iter != type_tbl.end(), "this chain type is not existed");
        futureio_assert(genesis_producer_pubkey.size() == 64, "subchain genesis producer public key should be of size 64");
    ...
    /// @abi action
    void system_contract::acceptmaster(const std::vector<futureio::signed_block_header>& headers) {
        futureio_assert(!_gstate.is_master_chain(), "master chain can not perform this action");
    ...
    bool system_contract::check_block_proposer(account_name block_proposer, chains_table::const_iterator chain_iter) {
        ...
        if(block_proposer != N(futio.empty) && block_proposer != N(genesis)) {
    ...
    void system_contract::acceptheader (name chain_name,
                                  const std::vector<futureio::signed_block_header>& headers) {
        futureio_assert(!headers.empty(), "at least one block should be contained");
        futureio_assert(headers.size() <= 10, "too many blocks are reported.");
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "subchian is not existed.");
        ...
        if( _producer_chain.end() == prod ) {
            ...
            futureio_assert(valid_sender, "invalid sender");
        } else if( ite_chain->is_prod_supervision() && fixed_cmt.find( prod->owner ) == fixed_cmt.end() ) {
        ...
        for(uint32_t idx = 0; idx < headers.size(); ++idx) {
            ...
            futureio_assert(block_number > ite_chain->confirmed_block_number, "block has been confirmed");
            ...
            if(block_proposer == N(genesis) && block_number > 1) {
                futureio_assert(headers[idx].signature.size() == 128, "signature should be of size 128");
            }
            ...
            if(block_number > initial_block_number) {
                futureio_assert(pre_block_index < ite_chain->unconfirmed_blocks.size(), "previous block is not found\n");
            }
            ...
            if((block_proposer == N(genesis) && block_number != initial_block_number) ||
                ...
                char confirm_id[32];
                futureio_assert(accept_block_header(chain_name, headers[idx], confirm_id, sizeof(confirm_id)), "light client check failed");
                        ...
                        if(block_ite != subchain_block_tbl.end()) {
                            ...
                            if(ncs.exists()) {
                                ...
                                if(new_chain.chain_name == chain_name
                                   ...
                                   && ite_uncfm_block->proposer != N(futio.empty)) {
        ...
        futureio_assert(block_number - ite_chain->confirmed_block_number <= 100, "too many unconfirmed blocks");
    }

    void system_contract::clearchain(name chain_name, bool users_only) {
        require_auth(N(futureio));
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "clearchain: this chian is not existed.");
        if(users_only) {
            _chains.modify(ite_chain, [&]( auto& _subchain ) {
    ...
    struct resourceleaseparam {
        ...
        FUTURELIB_SERIALIZE(resourceleaseparam, (from)(receiver)(combosize)(period)(location))
    };
    ...
    void system_contract::empoweruser(account_name user, name chain_name, const std::string& owner_pk, const std::string& active_pk, bool updateable) {
        ...
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "this subchian is not existed");
        futureio_assert(!is_empowered(user, chain_name), "user has been empowered to this chain before");
        auto briefprod = _briefproducers.find(user);
        bool is_prod = true;
        if(briefprod == _briefproducers.end()) {
            is_prod = false;
        }
        futureio_assert(owner_pk.size() == 53, "owner public key should be of size 53");
        futureio_assert(active_pk.size() == 53, "avtive public key should be of size 53");
        ...
        if(user == new_chain.first_period_buyer) {
            ...
            resourceleaseparam newchainresparam(N(futureio), user, 10000, 1, chain_name);
            uint128_t sendid = _self + N(newchainres) + chain_name;
            cancel_deferred(sendid);
            futureio::transaction out;
            out.actions.emplace_back( permission_level{ _self, N(active) }, N(futio.res), NEX(resourcelease), newchainresparam );
            out.delay_sec = 0;
    ...
    void system_contract::empower(account_name user, name chain_name, bool updateable) {
        ...
        futureio_assert( _gstate.is_master_chain(), "only master chain allow empower" );
        futureio_assert(!is_empowered(user, chain_name), "this account has been empowered to the chain");
    ...
    void system_contract::add_to_chain(name chain_name, const producer_info& producer, uint64_t current_block_number, bool with_heartbeat) {
        ...
        if(chain_name == default_chain_name) {
            ...
            auto ite_chain = _chains.find(chain_name);
            futureio_assert(ite_chain != _chains.end(), "destination chain is not existed" );
            chaintypes_table type_tbl(_self, _self);
            auto typeiter = type_tbl.find(ite_chain->chain_type);
            futureio_assert(typeiter != type_tbl.end(), "destination chain type is not existed");
            futureio_assert(ite_chain->committee_num < typeiter->stable_max_producers,
                "destination chain already has enough producers");
            //check if this account has been empowered to this chain
            futureio_assert(is_empowered(producer.owner, chain_name), "account has not been empower to destination chain");
        ...
        futureio_assert(prod == _producer_chain.end(), "producer has existed in destination chain");
    ...
    void system_contract::remove_from_chain(name chain_name, account_name producer_name, uint64_t current_block_number) {
        ...
        futureio_assert(prod != _producer_chain.end(), "producer is not existed in source chain");

        if(chain_name == default_chain_name) {
            new_chain_singleton  ncs(_self, _self);
            futureio_assert(ncs.exists(), "new chain info is null");
            auto new_chain = ncs.get();
            futureio_assert(new_chain.pending_producer_num > 0, "pending producer user number is wrong");
            new_chain.pending_producer_num -= 1;
            ncs.set(new_chain);
        } else if (chain_name == self_chain_name) {
            futureio_assert(_gstate.cur_committee_number > _gstate.min_committee_member_number, "The current number of committees must be greater than the minimum number");
            _gstate.cur_committee_number--;
        } else{
            auto ite_chain = _chains.find(chain_name);
            futureio_assert(ite_chain != _chains.end(), "source sidechain is not existed");
            chaintypes_table type_tbl(_self, _self);
            auto typeiter = type_tbl.find(ite_chain->chain_type);
            futureio_assert(typeiter != type_tbl.end(), "source chain type is not existed");
            ...
            if(!in_destory) {
                futureio_assert(ite_chain->committee_num > typeiter->stable_min_producers,
    ...
    void system_contract::pre_schedule(uint64_t current_block_height) {
        ...
        auto max_minutes = getglobalextenuintdata(futureio_global_state::pending_producer_max_minutes, 43200);//30 days as default: 30*24*60
    ...
    void system_contract::setsched(bool is_enabled, uint16_t sched_period, uint16_t expire_time) {
        ...
        futureio_assert(sched_period <= 1440*7, "scheduling period is overlong, it's supposed at least once a week."); //60m*24h = minutes per day, perform scheduling at least once a week;
        futureio_assert(sched_period >= 15, "scheduling period should be at least 15 minutes");
        futureio_assert(expire_time <= 30, "committee update confirm time is overlong, its maximum time is 30 minutes");
        futureio_assert(expire_time < sched_period, "committee update confirm time must lesser than scheduling period");
    ...
    void system_contract::forcesetblock(name chain_name, const signed_block_header& signed_header, const std::vector<committee_info>& cmt_set) {
        ...
        require_auth(N(futureio));
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "chain is not found");
    ...
    name system_contract::get_default_chain() {
        ...
        for(; ite_chain != _chains.end(); ++ite_chain) {
            ...
            futureio_assert(ite_type != type_tbl.end(), "error: chain type is not existed");
        ...
        futureio_assert(ite_min != _chains.end(), "currently no available chain for new producer to join");
    ...
    uint32_t system_contract::find_previous_block(const std::vector<unconfirmed_block_header>& block_vct, uint32_t block_num,
        ...
        do{
            ...
            if(block_vct[i].block_number == block_num) {
                //duplicated block
                futureio_assert(block_vct[i].block_id != block_id, "block has been accepted");
            }
    ...
    uint64_t system_contract::get_initial_block_num(name chain_name) {
        if(chain_name == N(master)) {
            master_chain_infos masterinfos(_self, _self);
            auto initmasteriter = masterinfos.find(N(futureio));
            futureio_assert(initmasteriter != masterinfos.end(), "master_chain_infos is not existed.");
    ...
    void system_contract::handle_new_confirm_block(chain_info& _chain, const block_id_type& confirm_block_id) {
        ...
        futureio_assert(ite_confirm_block != _chain.unconfirmed_blocks.end(), "error, confirm block is not found");
    ...
    void system_contract::setchainparam(name chain_name, uint64_t chain_type, bool is_sched_on, bool prod_supervision) {
        require_auth(N(futureio));
        futureio_assert(_gstate.is_master_chain(), "only master chain can perform this action");
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "chain is not found");
    ...
    void system_contract::setgenesisprodpk(uint64_t chain_type, const std::string& genesis_prod_pk) {
        ...
        futureio_assert(typeiter != type_tbl.end(), "chain type does not exist");
        ...
        if(!ncs.exists()) {
            ...
            futureio_assert(new_chain_info::new_chain_status::empty == _new_chain.creation_status
            ...
            _new_chain.first_period_buyer = N(futureio);
    ...
    void system_contract::startnewchain(name chain_name, account_name owner) {
        ...
        futureio_assert(itor == _chains.end(), "there has been a subchian with this name");
        new_chain_singleton  ncs(_self, _self);
        futureio_assert(ncs.exists(), "new chain info is empty, please contact Future to set it");
        auto new_chain = ncs.get();
        futureio_assert(new_chain_info::new_chain_status::auxiliary_nodes_prepared == new_chain.creation_status, "wrong new chain status");
        ...
        futureio_assert(resourcefee > 0, "resource lease resourcefee is abnormal" );
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {owner,N(active)},
                            { owner, N(futureio), asset(resourcefee), std::string("buy new chain resource") } );
        print("startnewchain: new chain buyer", name{owner}," chain name", chain_name, " resourcefee:",resourcefee);

        //register new chain
        INLINE_ACTION_SENDER(futureiosystem::system_contract, regsubchain)(N(futureio), {N(futureio), N(active)},
    ...
    void system_contract::schedule_pending_prod_to_newchain(uint64_t current_block_height) {
        ...
        char opk_buf[54];
        char apk_buf[54];
        auto min_minutes = getglobalextenuintdata(futureio_global_state::pending_producer_min_minutes, 10);
    ...
    void system_contract::destorychain(name chain_name, bool force) {
        require_auth(N(futureio));
        auto ite_chain = _chains.find(chain_name);
        futureio_assert(ite_chain != _chains.end(), "destory chain: this chian is not existed.");
        //check token
        chainbalance  chainbalan(N(futio.bank), N(futio.bank));
        auto it_bank = chainbalan.find( chain_name );
        if(it_bank != chainbalan.end()) {
            if(!force) {
                futureio_assert(it_bank->balance.amount == 0, "can not destory chain because there are tokens in side chain");
            } else if (it_bank->balance.amount > 0) {
                //TODO, add inline action in futio.bank to clear chainbalance
            }
        }
    ...
    void system_contract::check_producer_heartbeat(uint64_t current_block_height) {
        auto period_minutes = getglobalextenuintdata(futureio_global_state::producer_heartbeat_check_period, 10);
        ...
        for(; chain_iter != _chains.end(); ++chain_iter) {
            ...
            for(; prod_iter != _producers.end(); ++prod_iter) {
                if( fixed_cmt.find( prod_iter->owner ) != fixed_cmt.end() ) {
                    continue;
                }
