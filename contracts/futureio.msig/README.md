...
Create a proposal
## futureio.msig::propose    proposer proposal_name requested trx
...
Approve a proposal
## futureio.msig::approve    proposer proposal_name level
   - **proposer** account proposing a transaction
...
## futureio.msig::unapprove    proposer proposal_name level
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Cancel a proposal
## futureio.msig::cancel    proposer proposal_name canceler
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **canceler** account canceling the transaction (only proposer can cancel not expired transaction)

Execute a proposal
## futureio.msig::exec    proposer proposal_name executer
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **executer** account executing the transaction


Clfuture usage example.

Prerequisites:
   - futureio.token contract installed to futureio.token account, futureio.msig contract installed on futureio.msig account which is a privileged account.
   - account 'treasury' is the issuer of FUTURE token.
...
````
$ clfuture multisig propose test '[{"actor": "treasury", "permission": "active"}]' '[{"actor": "treasury", "permission": "active"}]' futureio.token issue '{"to": "tester", "quantity": "1000.0000 FUT", "memo": ""}' -p tester
executed transaction: e26f3a3a7cba524a7b15a0b6c77c7daa73d3ba9bf84e83f9c2cdf27fcb183d61  336 bytes  107520 cycles
#    futureio.msig <= futureio.msig::propose          {"proposer":"tester","proposal_name":"test","requested":[{"actor":"treasury","permission":"active"}]...
````

Another user reviews the transaction:
````
$ clfuture multisig review tester test -p treasury
{
  "proposal_name": "test",
  "requested_approvals": [{
      "actor": "treasury",
...
````
$ clfuture multisig approve tester test '{"actor": "treasury", "permission": "active"}' -p treasury
executed transaction: 475970a4b0016368d0503d1ce01577376f91f5a5ba63dd4353683bd95101b88d  256 bytes  108544 cycles
#    futureio.msig <= futureio.msig::approve          {"proposer":"tester","proposal_name":"test","level":{"actor":"treasury","permission":"active"}}
````

First user initiates execution:
````
$ clfuture multisig exec tester test -p tester
executed transaction: 64e5eaceb77362694055f572ae35876111e87b637a55250de315b1b55e56d6c2  248 bytes  109568 cycles
#    futureio.msig <= futureio.msig::exec             {"proposer":"tester","proposal_name":"test","executer":"tester"}
````
