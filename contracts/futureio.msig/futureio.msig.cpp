#include <futureio.msig/futureio.msig.hpp>
#include <futureiolib/action.hpp>
#include <futureiolib/permission.hpp>
...
void multisig::propose() {
   ...
   futureio_assert( proposer == N(futiomsig), " only allow futiomsig propose" );
   futureio_assert( trx_header.expiration >= futureio::time_point_sec(now()), "transaction expired" );
   //futureio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   futureio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   futureio_assert( res > 0, "transaction authorization failed" );

   uint32_t feeNum = (size * 100000) / 1024; //10FGAS/K, 10FGAS = asset(100000)
   if(feeNum < 1000) {
       feeNum = 1000;
   }
   INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {proposer,N(active)},
                        { proposer, N(futio.fee), asset(feeNum), std::string("ram fee") } );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   futureio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );

   INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {level.actor,N(active)},
                        { level.actor, N(futio.fee), asset(1000), std::string("ram fee") } );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   futureio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      futureio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < futureio::time_point_sec(now()), "cannot cancel until expiration" );
   }

   INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {canceler,N(active)},
                        { canceler, N(futio.fee), asset(1000), std::string("ram fee") } );
...
void multisig::exec( account_name proposer, name proposal_name, account_name executer ) {
   ...
   futureio_assert( res > 0, "transaction authorization failed" );
   INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {executer,N(active)},
                           { executer, N(futio.fee), asset(1000), std::string("ram fee") } );
...
FUTUREIO_ABI( futureio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
