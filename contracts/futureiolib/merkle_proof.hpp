...
namespace futureio {
    struct stateful_transaction : public transaction {
        ...
        FUTURELIB_SERIALIZE_DERIVED( stateful_transaction, transaction, (status)(tx_id) )
    };
    ...
    struct merkle_proof {
        ...
        stateful_transaction recover_transaction() {
            ...
            futureio_assert(ret != -1, "transaction_receipt only contains transaction id, no packed_transaction contains.");
            if (ret != 0) {
                size = (size_t)ret;
                buffer = malloc(size);
                ret = ts_recover_transaction(buffer, size, (const char*)(&tx_bytes[0]), tx_bytes.size());
                futureio_assert(ret == 0, "read packed_transaction raw data failed.");
            }
        ...
        static merkle_proof get_merkle_proof(uint32_t block_number, std::string tx_id) {
            ...
            if (ret != 0) {
                ...
                futureio_assert(ret == 0, "read merkle proof failed.");
            }
            merkle_proof mklp;
            datastream<char*> ds( (char*)buffer, size );
            ds >> mklp.proofs;
