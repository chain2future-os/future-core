...
namespace futureio {
   ...
   void set_blockchain_parameters(const futureio::blockchain_parameters& params) {
      char buf[sizeof(futureio::blockchain_parameters)];
      futureio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(futureio::blockchain_parameters& params) {
      char buf[sizeof(futureio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      futureio_assert( size <= sizeof(buf), "buffer is too small" );
      futureio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            futureio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return futureio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = futureio::memory_heap.malloc(count*size);
...
{
   return futureio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return futureio::memory_heap.free(ptr);
}
