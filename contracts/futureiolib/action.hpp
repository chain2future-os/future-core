...
namespace futureio {
   ...
   struct permission_level {
      ...
      FUTURELIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct proposeaccount_info {
      ...
      name              location;
      int64_t           approve_num;
      FUTURELIB_SERIALIZE( proposeaccount_info, (account)(owner_key)(active_key)(updateable)(location)(approve_num) )
   };
   ...
   struct block_reward {
       ...
       uint64_t         reward;
       FUTURELIB_SERIALIZE(block_reward , (consensus_period)(reward) )
   };
   ...
   struct action {
      ...
      FUTURELIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         futureio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         futureio_assert( name == T::get_name(), "Invalid name" );
         futureio_assert( account == T::get_account(), "Invalid account" );
...
::futureio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), (NEX(ACTION_NAME).valueH), (NEX(ACTION_NAME).valueL)>::call
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::futureio::action_meta<CODE, (NEX(NAME).valueH), (NEX(NAME).valueL)>
