#pragma once
#include <futureiolib/core_symbol.hpp>
...
namespace futureio {
   ...
   */
    ...
    * @param str - the string representation of the symbol
    */
   #define S(P,X) ::futureio::string_to_symbol(P,#X)
   ...
   /**
    * Returns the symbol name as string
    *
    * @param sym - symbol name (uint64_t) without precision
    * @Note: sym does not contain precision
    */
   std::string symbol_name_to_string( symbol_name sym ) {
      std::string result;
      while (sym > 0) {
         char c = (char)(sym & 0xFF);
         result += c;
   ...
   struct symbol_type {
      ...
      */
      ...
      FUTURELIB_SERIALIZE( symbol_type, (value) )
   };
   ...
   struct extended_symbol : public symbol_type
   {
     ...
     extended_symbol( symbol_name sym = 0, account_name acc = 0 ):symbol_type{sym},contract(acc){}
      ...
      FUTURELIB_SERIALIZE( extended_symbol, (value)(contract) )
   };
