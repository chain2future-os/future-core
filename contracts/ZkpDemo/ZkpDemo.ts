...
import { verify_zero_knowledge_proof } from "future-ts-lib/src/crypto";
import { SHA256 } from "future-ts-lib/src/crypto";
import { NAME } from "future-ts-lib/src/account";
import { Log } from "future-ts-lib/src/log";
...
export　class ZkpDemo extends Contract {
	...
	pm_record.hash = ds.read<u64>();
        ...
        future_assert(!this.pmManager.exists(pm_record.hash), "the primary input already exists");
        ...
        future_assert(record.val == expect, "The zkp test case result is not expect.")
        if (this.resultManager.exists(record.id)) {
            future_assert(false, "The test case already exists");
        } else {
    ...
    private getVerifyKey(id: u64): string {
        ...
        future_assert(false, "Please set the verify key.");
        return "";
    }
}
