...
namespace futureiores {
    ...
    void resource::resourcelease( account_name from, account_name receiver,
        ...
        futureiosystem::global_state_singleton   gstatesingle(N(futureio),N(futureio));
        futureio_assert( gstatesingle.exists(), "global table not found");
        futureiosystem::futureio_global_state globalst = gstatesingle.get();
        if( !globalst.is_master_chain() || !_gstate.is_allow_buy_res )
            futureio_assert( from == futureio_account_name, "only allow futureio account resourcelease" );
        ...
        futureio_assert(combosize <= _gstate.max_resources_number, assert_des.c_str());

        if(!has_auth(futureio_account_name)) {
            require_auth( from );
            futureio_assert(combosize == _gstate.max_resources_number, ("must buy full resource: " + std::to_string(_gstate.max_resources_number) ).c_str());
        }
        ...
        if(ite_period != _resperiods_tbl.end()) {
            futureio_assert( period >= cur_period, ("expired period, current period is " + std::to_string(cur_period) ).c_str());
            futureio_assert(ite_period->total_lease_num + combosize <= _gstate.max_resources_number, "total combosize of this period exceeds full size");
            futureio_assert((from == futureio_account_name) || (ite_period->owner == receiver), "only period owner can continue to receive resource of this period");
            ...
            futureio_assert( should_buy_period == period, ("excepted period: " + std::to_string(should_buy_period) + " is not equal to required period: " + std::to_string(period)).c_str() );
        ...
        if ( globalst.is_master_chain() ) {
            ...
            if( from != futureio_account_name ) { //futureio resourcelease not charge
                ...
                INLINE_ACTION_SENDER(futureiosystem::system_contract, updatecommercdel)( N(futureio), {N(futureio),N(active)},
    ...
    void resource::transresource(account_name from, account_name to, uint16_t combosize, uint64_t period){
        require_auth( from );
        futureio_assert(from != to, "can't transfer resource to oneself");
        futureio_assert(combosize > 0, "resource combosize must be greater than 0" );
        ...
        futureio_assert(lease_from.lease_num >= combosize, ( name{from}.to_string() + " has not enough resources to transfer ").c_str() );
        if ( period == 0 ) {
            period = cur_period;
        }
        ...
        if ( period == cur_period ) { //Process current period resources
            ...
            futureio_assert(need_ram <= total_ram - used_ram, "don't have enough rest resource of current period to transfer");
        ...
        futureio_assert(res_trans_fee > 0, "resource lease res_trans_fee is abnormal" );
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {from,N(active)},
                            { from, N(futio.fee), asset(res_trans_fee), std::string("transresource fee") } );
    }
    ...
    void resource::transaccount( account_name from, account_name to, uint32_t number ) {
        ...
        futureio_assert( resacc_itr !=  _resacc_tbl.end(), ( name{from}.to_string() + " has no account in chain ").c_str() );
        futureio_assert( resacc_itr->free_account_number >= number, ( name{from}.to_string() + " has not enough account to transfer ").c_str() );
        futureio_assert(number > 0, "transfer account must be greater than 0" );
        ...
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {from,N(active)},
                            { from, N(futio.fee), asset(10000), std::string("transfer account fee") } );
    }
    ...
    void resource::modifyfreeaccount( const account_name owner, uint32_t number) {
        require_auth( _self );
        futureio_assert( number > 0, "modifyfreeaccount number is abnormal" );
        resfreeaccount _resacc_tbl( _self, _self );
        auto resacc_itr = _resacc_tbl.find( owner );
        futureio_assert( resacc_itr !=  _resacc_tbl.end(), "The current free account is insufficient, please purchase free account" );
        futureio_assert( resacc_itr->free_account_number >= number, "Insufficient number of transfer free accounts" );
        if ( resacc_itr->free_account_number == number ) {
            _resacc_tbl.erase( resacc_itr );
            return;
        }
...
/*
    void resource::copyresource() {
        ...
        futureiosystem::global_state_singleton   gstatesingle(N(futureio),N(futureio));
        futureiosystem::futureio_global_state globalst = gstatesingle.get();
        futureiosystem::resources_lease_table _legacy_resource( N(futureio), N(futureio) ); //copy self_chain first
        ...
        for(; legacy_ite != _legacy_resource.cend(); ++legacy_ite) {
            ...
            if(legacy_ite->lease_num >= 5000) {
                resources_periods_table _resperiods_tbl( _self, N(futureio));
                _resperiods_tbl.emplace( [&]( auto& tot ) {
                    tot.periods = 1;
        ...
        futureiosystem::chains_table all_chains(N(futureio),N(futureio));
        ...
        for(; chain_ite != all_chains.cend(); ++chain_ite) {
            ...
            futureiosystem::resources_lease_table _legacy_sidechain_resource( N(futureio), chain_ite->chain_name );
    ...
    */
    ...
    void resource::putorder(account_name owner, uint64_t period, uint16_t combosize, asset price, bool decrease) {
        ...
        if(period > 0) {
            futureio_assert( period >= cur_period_id, "this order is out of date");
        ...
        futureio_assert(ite_res != _reslease_tbl.end(), "you don't have resource of this period");
        futureio_assert(ite_res->lease_num >= combosize, "you don't have enough resource");
        ...
        if(current_period) {
            ...
            uint64_t bytes_per_combo = _gstate.max_ram_size/_gstate.max_resources_number;
            int64_t need_ram = int64_t(bytes_per_combo * combosize);
            futureio_assert(need_ram <= total_ram - used_ram, "you don't have enough rest resource of current period");
        ...
        if(resorders.end() == ite_order) {
            futureio_assert(price.amount > 0, "you dont't have an existed order, so the price cannot be set as zero");
            ...
            if(price.amount > 0) {
                futureio_assert(ite_order->decrease_by_day == decrease, "you already have an order with different price strategy");
            }
            ...
            if(price.amount > 0) {
                ...
                futureio_assert(realtime_unit_price == uint64_t(price.amount), assert_msg.c_str());
            }
        ...
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {owner, N(active)},
                    { owner, N(futio.fee), asset(1000), std::string("put order") } );
    ...
    void resource::updateorder(account_name owner, uint64_t period, asset price, bool decrease) {
        require_auth(owner);
        futureio_assert(price.amount > 0, "price cannot be set as zero");
        uint32_t cur_block_height = (uint32_t)head_block_number() + 1;
        uint64_t real_period = period;
        uint64_t cur_period_id = cur_block_height * block_interval_seconds()/ seconds_per_period + 1;
        if(0 == period) {
        ...
        ressaletable resorders(_self, real_period);
        auto ite_order = resorders.find(owner);
        futureio_assert(ite_order != resorders.end(), "order is not found");
        resorders.modify(ite_order, [&]( auto& order ) {
            order.initial_unit_price = uint64_t(price.amount);
            order.decrease_by_day = decrease;
            order.modify_block_height = cur_block_height;
        ...
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {owner, N(active)},
                    { owner, N(futio.fee), asset(200), std::string("update order") } );
    }

    void resource::cancelorder(account_name owner, uint64_t period, uint16_t combosize) {
        require_auth(owner);
        futureio_assert(combosize > 0, "combosize must be greater than 0");
        uint32_t cur_block_height = (uint32_t)head_block_number() + 1;
        uint64_t real_period = period;
        uint64_t cur_period_id = cur_block_height * block_interval_seconds()/ seconds_per_period + 1;
        bool current_period = true;
        ...
        auto ite_order = resorders.find(owner);
        futureio_assert(ite_order != resorders.end(), "order is not found");
        futureio_assert(ite_order->lease_num >= combosize, "required combosize exceeds resource number in the order");

        resources_lease_table _reslease_tbl( _self, period );
        auto ite_res = _reslease_tbl.find(owner);
        futureio_assert(ite_res != _reslease_tbl.end(), "error: resource is not found");
        futureio_assert(ite_order->lease_num == ite_res->locked_num, "resource size in the order is mismatch with locked size");
    ...
    void resource::buyin(account_name owner, uint64_t period, account_name buyer, uint16_t combosize) {
        ...
        auto ite_order = resorders.find(owner);
        futureio_assert(ite_order != resorders.end(), "order is not found");
        futureio_assert(ite_order->lease_num >= combosize, "the required combo size is more than the offered size");
        ...
        INLINE_ACTION_SENDER(futureio::token, transfer)( N(futio.token), {buyer, N(active)},
                    { buyer, N(futio.fee), asset(1000), std::string("buy resource order") } );
...
FUTUREIO_ABI( futureiores::resource, (setresparams)(resourcelease)(transresource)(transaccount)(recycleresource)(onblock)(modifyfreeaccount)(putorder)(updateorder)(cancelorder)(buyin) )
