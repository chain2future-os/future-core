import { Contract } from "future-ts-lib/src/contract";
import { RNAME, Account } from "future-ts-lib/src/account";
import { Action } from "future-ts-lib/src/action";
import { Block } from "future-ts-lib/src/block";
import { intToString } from "future-ts-lib/src/utils";
import { Asset } from "future-ts-lib/src/asset";
import { verify_with_pk, SHA256 } from "future-ts-lib/src/crypto";
import {
  Voter,
  Waiter,
  RandRecord,
...
class RandContract extends Contract {
  ...
  private register(user: account_name, value: Asset, memo: string): void {
    ...
    future_assert(!this.voteDB.exists(user), "cannot add existing candidate.");
        
    if (!value.eq(WAITER_DEPOSIT_AMOUNT)) {
      future_assert(false, "deposit money is not accurate, require deposit 1000 FGAS");
    }
    ...
    future_assert(waitorseq.waiters.length <= <i32>WAITER_NUM, "The total of waiters should be less than " + intToString(WAITER_NUM));
  ...
  @action
  unregister(): void {
    future_assert(this.voteDB.exists(Action.sender), "cannot unregister non-existing voter.");
    var voter = new Voter();
    this.voteDB.get(Action.sender, voter);
    future_assert(!voter.isUnregister(), "cannot repeat to unregsiter voter");
  ...
  @action
  redeem(): void {
    future_assert(this.voteDB.exists(Action.sender), "cannot redeem non-existing voter.");
    var voter = new Voter();
    this.voteDB.get(Action.sender, voter);
    if (voter.redeemable()) {
      ...
      future_assert(false, "cannot redeem deposit, you must wait for some blocks generated or unregister first.");
    }
  }

  private getExistRand(blockNum: u64): u64 {
    var rand = new RandRecord();
    future_assert(this.randDB.exists(blockNum), "The random of block height has not generated.");
  ...
  vote(pk_proof: string, blockNum: u64): void {
    ...
    future_assert(this.belongRandNum(blockNum) >= curtBckNum, "The current block number is " 
    ...
    if (this.random.isMainVoter(sender, blockNum)) {
      ...
      future_assert(this.voteDB.exists(sender), "You " + RNAME(sender) + " should be a candidate first.");
      var voter = new Voter();
      this.voteDB.get(sender, voter);
      future_assert(voter.votable(), "The voter " + RNAME(sender) + " status is not votable.");
  ...
  private saveVoterVoteStatus(name: account_name, seedBckNum: u64, vrf: u64): void {
    ...
    if (this.voteDB.exists(name)) {
      ...
      if (this.belongRandNum(seedBckNum) == voteInfo.belongBckNums[index]) {
        future_assert(false, "You had already submitted the vote, you should wait for the next round.");
      }
      if (this.belongRandNum(seedBckNum) >= curtBlockNum || voteInfo.belongBckNums[index] == 0) {
        ...
        future_assert(false, "Your vote was expired.");
      }
    } 
  }
  ...
  private getVrfVal(pk_proof: string, blockNum: u64): u64 {
    ...
    future_assert(verify_with_pk(pkStr, pk_proof, seedStr), "please provide a valid VRF proof." + pkStr + " " + pk_proof + " " + seedStr);
  ...
  private mainVoterVote(name: account_name, pk_proof: string, blockNum: u64): void {
    if (this.random.isMainVoted(name, blockNum)){
      future_assert(false, RNAME(name) + " had already submitted the vote, please wait for the next round.");
    }
  ...
  private waiterVoterVote(name: account_name, pk_proof: string, blockNum: u64): void {
    if (this.curtBckNum() - blockNum > 2) {
      future_assert(false, "The waiter should vote the next two block period after the blockNum");
    }
    var vrf = this.getVrfVal(pk_proof, blockNum);
