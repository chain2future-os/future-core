                    ...
                    asset        maximum_supply )
{
    ...
    futureio_assert( maximum_supply.is_valid(), "invalid supply");
    futureio_assert( maximum_supply.amount > 0, "max-supply must be positive");
...
void token::issue( account_name to, asset quantity, string memo )
{
    ...
    futureio_assert( sym.is_valid(), "invalid symbol name" );
    futureio_assert( memo.size() <= 256, "memo has more than 256 bytes" );

    auto sym_name = sym.name();
    stats statstable( _self, sym_name );
    auto existing = statstable.find( sym_name );
    futureio_assert( existing != statstable.end(), "token with symbol does not exist, create token before issue" );
    const auto& st = *existing;

    require_auth( st.issuer );
    futureio_assert( quantity.is_valid(), "invalid quantity" );
    futureio_assert( quantity.amount > 0, "must issue positive quantity" );

    futureio_assert( quantity.symbol == st.supply.symbol, "symbol precision mismatch" );
    futureio_assert( quantity.amount <= st.max_supply.amount - st.supply.amount, "quantity exceeds available supply");
...
void token::burn( asset quantity )
{
    ...
    stats statstable( _self, sym_name );
    auto existing = statstable.find( sym_name );
    futureio_assert( existing != statstable.end(), "token with symbol does not exist" );
    const auto& st = *existing;

    require_auth( st.issuer );
    futureio_assert( quantity.is_valid(), "invalid quantity" );
    futureio_assert( quantity.amount > 0, "must burn positive quantity" );
...
void token::transfer( account_name from,
                      ...
                      string       memo )
{
    futureio_assert( from != to, "cannot transfer to self" );
    require_auth( from );
    futureio_assert( is_account( to ), "to account does not exist");
    ...
    futureio_assert( !st.is_forbid_transfer, " transaction is not allowed" );
    futureio_assert( quantity.is_valid(), "invalid quantity" );
    futureio_assert( quantity.amount > 0, "must transfer positive quantity" );
    futureio_assert( quantity.symbol == st.supply.symbol, "symbol precision mismatch" );
    futureio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
void token::set_chargeparams( std::string symbol, uint8_t precision, uint32_t operate_interval, uint32_t operate_fee, bool is_forbid_trans)
{
    ...
    futureio_assert( stat != statstable.end(), "symbol not exists" );
...
void token::sub_balance( account_name owner, asset value ) {
   ...
   if( owner != N(futureio) && name{owner}.to_string().find( "futio." ) != 0 ){
      asset fee( get_transfer_fee() );
      futureio_assert( fee.amount > 0, "fee must a positive value" );
      add_balance( N(futio.fee), fee );
      value += fee;
   }

   futureio_assert( from.balance.amount >= value.amount, "there may be a charge for this account, but overdrawn balance" );
...
void token::set_trans_fee( int64_t fee ) {
   ...
   futureio_assert( itr_fee == _fee.end(), " set fee block height already exist ");
   _fee.emplace( [&]( auto& f ) {
...
FUTUREIO_ABI( futureio::token, (create)(issue)(burn)(transfer)(set_chargeparams)(set_trans_fee) )
