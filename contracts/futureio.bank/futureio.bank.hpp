...
#include <futureiolib/currency.hpp>
#include<string>
#include<vector>
namespace futureiobank {
    ...
    class bank : public contract {
        ...
        private:
            struct bulletin_info {
                ...
                asset        quantity;
                FUTURELIB_SERIALIZE(bulletin_info, (receiver)(quantity))
            };
            struct bulletin_bank {
                ...
                FUTURELIB_SERIALIZE(bulletin_bank, (block_height)(bulletin_infos))
            };
            typedef futureio::multi_index<N(bulletinbank), bulletin_bank> bulletinbank;
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t actH, uint64_t actL ) {
        ...
        if( code == N(futio.token) && action == NEX(transfer)){
            bank bk(receiver);
