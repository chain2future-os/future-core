...
#include <futureio/chain/block_state.hpp>
...
namespace futureio { namespace chain { namespace config {
           template<>
...
using namespace futureio::chain::resource_limits;
...
void read_section(std::ostream& out,ProcessTable&& pt) {
    auto it = [&](auto &section){
            ...
            while(more) {
              total_record++;
              auto row = typename futureio::chain::detail::worldstate_row_traits<ObjectType>::worldstate_type();
...
void decode_ws(std::ostream& out){
    ...
    auto acc_obj=[&]LAMBDA_PARAME{
        ...
        FUTURE_ASSERT(account_info_map.find(row.name) == account_info_map.end(), worldstate_exception, "Account already exist: ${a}", ("a", row.name));
    ...
    auto tb = [&]LAMBDA_PARAME{
        ...
        FUTURE_ASSERT(account_info_map.find(row.code) != account_info_map.end(), worldstate_exception, "Account don't exist: ${a}", ("a", row.code));
    ...
    READ_SECTION_LAMBDA(account_sequence_object, [&]LAMBDA_PARAME{
        FUTURE_ASSERT(account_info_map.find(row.name) != account_info_map.end(), worldstate_exception, "Account don't exist: ${a}", ("a", row.name));
    ...
    READ_SECTION_LAMBDA(resource_limits_object, [&]LAMBDA_PARAME{
        FUTURE_ASSERT(account_info_map.find(row.owner) != account_info_map.end(), worldstate_exception, "Account don't exist: ${a}", ("a", row.owner));
